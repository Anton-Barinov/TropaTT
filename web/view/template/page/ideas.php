...
document.getElementById('addCommentBtn').addEventListener('click',function(){var input=document.getElementById('commentInput');var text=input.value.trim();if(!text)return;var b=this;b.disabled=true;window.CRM.api.request('api/v1/ideas/'+pid+'/comments',{method:'POST',body:{text:text}}).then(function(){input.value='';refreshVisualEditorScope(input.parentElement||document);b.disabled=false;loadComments();}).catch(function(){b.disabled=false;if(window.CRM.br1)window.CRM.br1.notify('error','<?= htmlspecialchars($t('ideas.error_send', 'Ошибка отправки'), ENT_QUOTES, 'UTF-8') ?>');});});
// Delete
