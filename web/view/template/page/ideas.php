...
function renderAnswerChoices(qid,options,allowCustom){
  ...
  h+='<button type="button" class="btn crm-inline-answer-save iv-custom-save" title="<?= htmlspecialchars($t('ideas.btn_save_custom_answer', 'Сохранить свой ответ'), ENT_QUOTES, 'UTF-8') ?>" aria-label="<?= htmlspecialchars($t('ideas.btn_save_custom_answer', 'Сохранить свой ответ'), ENT_QUOTES, 'UTF-8') ?>"><i class="fa-solid fa-check"></i></button>';
  ...
  if(window.CRM&&window.CRM.api){
    window.CRM.api.request('api/v1/ideas/'+pid+'/questions',{method:'GET'}).then(function(env){
      ...
      if(unanswered.length>0){
        ...
        b.innerHTML='<i class="fa-solid fa-comments me-1"></i> <?= htmlspecialchars($t('ideas.btn_ask_more', 'Задать ещё вопросы AI'), ENT_QUOTES, 'UTF-8') ?>';
      ...
      window.CRM.api.request('api/v1/ideas/'+pid+'/interview',{method:'POST',timeoutMs:180000}).then(function(env2){
        ...
        b.disabled=false;b.innerHTML='<i class="fa-solid fa-comments me-1"></i> <?= htmlspecialchars($t('ideas.btn_ask_more', 'Задать ещё вопросы AI'), ENT_QUOTES, 'UTF-8') ?>';loadInterview();
      }).catch(function(err){
        b.disabled=false;b.innerHTML='<i class="fa-solid fa-comments me-1"></i> <?= htmlspecialchars($t('ideas.btn_ask_ai', 'Задать вопросы AI'), ENT_QUOTES, 'UTF-8') ?>';
...
window._renderClarifications=function(data){
  ...
  h+='<button class="btn btn-sm crm-btn-primary mt-2" id="saveClarificationsBtn"><i class="fa-solid fa-paper-plane me-1"></i> <?= htmlspecialchars($t('ideas.btn_save_clarifications', 'Сохранить уточнения'), ENT_QUOTES, 'UTF-8') ?></button>';
  body.innerHTML=h;
  body.style.display='';
...
window._renderGaps=function(data){
  ...
  h+='<button class="btn btn-sm crm-btn-primary mt-2" id="saveGapBtn"><i class="fa-solid fa-paper-plane me-1"></i> <?= htmlspecialchars($t('ideas.btn_save_clarifications', 'Сохранить уточнения'), ENT_QUOTES, 'UTF-8') ?></button>';
  body.innerHTML=h;
  body.style.display='';
  bindQuestionAnswerUi(body);
...
<div class="row g-3 mb-3" id="leaderboardsRow" style="display:none">
  <div class="col-md-6">
    <div class="crm-card crm-section-card"><div class="crm-section-head"><h6 class="mb-0" data-i18n="ideas.leaderboard_votes_title"><i class="fa-solid fa-trophy text-warning me-1"></i> <?= htmlspecialchars($t('ideas.leaderboard_votes_title', 'Топ по голосам'), ENT_QUOTES, 'UTF-8') ?></h6></div>
    <div class="p-2"><div class="btn-group btn-group-sm w-100 mb-2" role="group" id="votesPeriodBtns">
      <button class="btn btn-outline-secondary active" data-period="today" data-i18n="ideas.period_today"><?= htmlspecialchars($t('ideas.period_today', 'Сегодня'), ENT_QUOTES, 'UTF-8') ?></button>
      <button class="btn btn-outline-secondary" data-period="week" data-i18n="ideas.period_week"><?= htmlspecialchars($t('ideas.period_week', 'Неделя'), ENT_QUOTES, 'UTF-8') ?></button>
      <button class="btn btn-outline-secondary" data-period="month" data-i18n="ideas.period_month"><?= htmlspecialchars($t('ideas.period_month', 'Месяц'), ENT_QUOTES, 'UTF-8') ?></button>
      <button class="btn btn-outline-secondary" data-period="all" data-i18n="ideas.period_all"><?= htmlspecialchars($t('ideas.period_all', 'Всё время'), ENT_QUOTES, 'UTF-8') ?></button>
    </div><div id="topVotesList" class="small"></div></div></div>
  </div>
  <div class="col-md-6">
    <div class="crm-card crm-section-card"><div class="crm-section-head"><h6 class="mb-0" data-i18n="ideas.leaderboard_comments_title"><i class="fa-solid fa-comments text-info me-1"></i> <?= htmlspecialchars($t('ideas.leaderboard_comments_title', 'Топ по комментариям'), ENT_QUOTES, 'UTF-8') ?></h6></div>
...
loadLeaderboards();
}).catch(function(){body.innerHTML='<tr><td colspan="7" class="text-danger"><?= htmlspecialchars($t('ideas.state_load_error', 'Ошибка загрузки'), ENT_QUOTES, 'UTF-8') ?></td></tr>';});}
...
window.CRM.api.request('api/v1/ideas',{method:'GET',query:{sort:'votes',period:period,limit:5}}).then(function(env){var items=env.data.items||[];var h='';if(!items.length){h='<div class="crm-empty-state py-2 px-2"><strong class="small"><?= htmlspecialchars($t('ideas.state_no_votes_period', 'За выбранный период голосов нет'), ENT_QUOTES, 'UTF-8') ?></strong><p class="text-muted small mb-0"><?= htmlspecialchars($t('ideas.state_ideas_may_exist', 'Идеи ниже могут быть заполнены, но в этом периоде по ним не голосовали.'), ENT_QUOTES, 'UTF-8') ?></p></div>';}else{items.forEach(function(i,n){h+='<div class="d-flex justify-content-between align-items-center py-1 border-bottom"><div><span class="badge bg-secondary me-1">'+(n+1)+'</span> <a href="index.php?route=idea-detail&id='+encodeURIComponent(i.public_id)+'" class="text-decoration-none small">'+window.CRM.text.escapeHtml(i.title||'')+'</a></div><span class="badge bg-success">'+i.vote_count+'</span></div>';});}document.getElementById('topVotesList').innerHTML=h;}).catch(function(){});}
function loadTopComments(period){
window.CRM.api.request('api/v1/ideas',{method:'GET',query:{sort:'comments',period:period,limit:5}}).then(function(env){var items=env.data.items||[];var h='';if(!items.length){h='<div class="crm-empty-state py-2 px-2"><strong class="small"><?= htmlspecialchars($t('ideas.state_no_comments_period', 'За выбранный период комментариев нет'), ENT_QUOTES, 'UTF-8') ?></strong><p class="text-muted small mb-0"><?= htmlspecialchars($t('ideas.state_comments_may_exist', 'Список идей ниже может быть заполнен, но обсуждений за этот период не было.'), ENT_QUOTES, 'UTF-8') ?></p></div>';}else{items.forEach(function(i,n){h+='<div class="d-flex justify-content-between align-items-center py-1 border-bottom"><div><span class="badge bg-secondary me-1">'+(n+1)+'</span> <a href="index.php?route=idea-detail&id='+encodeURIComponent(i.public_id)+'" class="text-decoration-none small">'+window.CRM.text.escapeHtml(i.title||'')+'</a></div><span class="badge bg-info">'+i.comment_count+'</span></div>';});}document.getElementById('topCommentsList').innerHTML=h;}).catch(function(){});}
