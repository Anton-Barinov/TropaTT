...
$L['ru'] = [
    ...
    'already_installed_desc' => 'Система уже установлена. Для переустановки удалите файлы: api/.env, api/.install.lock, storage_api/install.lock. Если нужно сохранить данные — не удаляйте api/.env, а вместо этого зайдите в раздел обновлений.',
    ...
    'db_not_exists_hint' => 'Убедитесь, что база данных создана в панели хостинга (MySQL).',
    'db_access_denied_hint' => 'Проверьте логин и пароль БД, а также права пользователя на запись.',
...
$L['en'] = [
    ...
    'already_installed_desc' => 'System is already installed. To reinstall, delete these files: api/.env, api/.install.lock, storage_api/install.lock. To keep your data, do not delete api/.env — use the Updates section instead.',
    ...
    'db_not_exists_hint' => 'Make sure the database was created in your hosting panel (MySQL).',
    'db_access_denied_hint' => 'Check the database username and password, and ensure the user has write privileges.',
...
$L['zh'] = [
    ...
    'already_installed_desc' => '系统已安装。如需重新安装，请删除以下文件：api/.env、api/.install.lock、storage_api/install.lock。如需保留数据，请勿删除 api/.env，而是使用更新功能。',
    ...
    'db_not_exists_hint' => '请确保已在主机面板（MySQL）中创建数据库。',
    'db_access_denied_hint' => '请检查数据库用户名和密码，并确保用户具有写入权限。',
...
$L['es'] = [
    ...
    'already_installed_desc' => 'El sistema ya está instalado. Para reinstalar, elimine estos archivos: api/.env, api/.install.lock, storage_api/install.lock. Para conservar los datos, no elimine api/.env — use la sección de Actualizaciones.',
    ...
    'db_not_exists_hint' => 'Asegúrese de que la base de datos fue creada en el panel de hosting (MySQL).',
    'db_access_denied_hint' => 'Verifique el nombre de usuario y contraseña de la BD, y asegúrese de que el usuario tenga permisos de escritura.',
...
$L['pt'] = [
    ...
    'already_installed_desc' => 'O sistema já está instalado. Para reinstalar, exclua estes arquivos: api/.env, api/.install.lock, storage_api/install.lock. Para manter os dados, não exclua api/.env — use a seção de Atualizações.',
    ...
    'db_not_exists_hint' => 'Certifique-se de que o banco de dados foi criado no painel de hospedagem (MySQL).',
    'db_access_denied_hint' => 'Verifique o nome de usuário e senha do BD, e certifique-se de que o usuário tenha permissões de escrita.',
...
$L['de'] = [
    ...
    'already_installed_desc' => 'System ist bereits installiert. Zum Neuinstallieren löschen Sie diese Dateien: api/.env, api/.install.lock, storage_api/install.lock. Um Daten zu behalten, löschen Sie api/.env nicht — verwenden Sie den Aktualisierungsbereich.',
    ...
    'db_not_exists_hint' => 'Stellen Sie sicher, dass die Datenbank im Hosting-Panel (MySQL) erstellt wurde.',
    'db_access_denied_hint' => 'Überprüfen Sie den Datenbank-Benutzernamen und das Passwort und stellen Sie sicher, dass der Benutzer Schreibrechte hat.',
...
$L['fr'] = [
    ...
    'already_installed_desc' => 'Le système est déjà installé. Pour réinstaller, supprimez ces fichiers : api/.env, api/.install.lock, storage_api/install.lock. Pour conserver les données, ne supprimez pas api/.env — utilisez la section Mises à jour.',
    ...
    'db_not_exists_hint' => "Assurez-vous que la base de données a été créée dans le panneau d'hébergement (MySQL).",
    'db_access_denied_hint' => 'Vérifiez le nom d\'utilisateur et le mot de passe de la BD, et assurez-vous que l\'utilisateur a des droits d\'écriture.',
    ...
    if ($action === 'install' && !empty($installData)) {
        try {
            ...
            try {
                ...
                $errMsg = $dbConnErr->getMessage();
                $hint = '';
                if (str_contains($errMsg, 'Unknown database')) {
                    $hint = ' — ' . t('db_not_exists_hint');
                } elseif (str_contains($errMsg, 'Access denied')) {
                    $hint = ' — ' . t('db_access_denied_hint');
                }
                echo json_encode([
                    'success' => false,
                    'substep' => $substep ?: 2,
                    'message' => t('db_connect_error') . ': ' . mb_substr($errMsg, 0, 200) . $hint,
