...
(function () {
  ...
  async function load() {
    ...
    document.getElementById('adminKnowledgeSpaces').innerHTML = adminSpaceList.map(function (space) {
      ...
      return '<tr><td data-label="' + esc(t('knowledge.field_title', 'Название')) + '"><strong>' + esc(space.title) + ' ' + archLabel + '</strong><div class="text-muted small">' + esc(space.description || '') + '</div></td><td data-label="' + esc(t('knowledge.visibility', 'Видимость')) + '">' + esc(space.visibility || '') + '</td><td data-label="' + esc(t('knowledge.stat_pages', 'Страниц')) + '">' + esc(space.pages_count || 0) + '</td><td data-label="' + esc(t('admin_knowledge.th_permissions', 'Доступ')) + '"><button class="btn btn-sm crm-btn-secondary" data-space-permissions="' + esc(space.public_id) + '" data-space-title="' + esc(space.title) + '">' + esc(t('admin_knowledge.btn_permissions', 'Доступ')) + '</button></td><td data-label="' + esc(t('common.actions', 'Действия')) + '" class="crm-table-actions"><div class="d-flex gap-1 justify-content-end">' + manageBtn + actionBtn + '</div></td></tr>';
