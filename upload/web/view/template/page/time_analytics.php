...
<div class="tab-content">
  ...
  <div id="timeAnalyticsEarnings" class="tab-pane fade">
    <div class="crm-toolbar-surface d-flex flex-wrap gap-2 align-items-center mb-3">
      ...
      <select id="timeAnalyticsEarningsClientFilter" class="form-select crm-field-w-220" aria-label="<?= htmlspecialchars($t('time_analytics.filter_client_aria', 'Контрагент'), ENT_QUOTES, 'UTF-8') ?>" data-i18n-aria-label="time_analytics.filter_client_aria"><option value="" data-i18n="time_analytics.opt_all_clients"><?= htmlspecialchars($t('time_analytics.opt_all_clients', 'Все контрагенты'), ENT_QUOTES, 'UTF-8') ?></option></select>
      <select id="timeAnalyticsEarningsActivityFilter" class="form-select crm-field-w-220" aria-label="<?= htmlspecialchars($t('time_analytics.filter_activity_aria', 'Вид работ'), ENT_QUOTES, 'UTF-8') ?>" data-i18n-aria-label="time_analytics.filter_activity_aria"><option value="" data-i18n="time_analytics.opt_all_activities"><?= htmlspecialchars($t('time_analytics.opt_all_activities', 'Все виды работ'), ENT_QUOTES, 'UTF-8') ?></option></select>
      <select id="timeAnalyticsEarningsUserFilter" class="form-select crm-field-w-220" aria-label="<?= htmlspecialchars($t('time_analytics.filter_user_aria', 'Пользователь'), ENT_QUOTES, 'UTF-8') ?>" data-i18n-aria-label="time_analytics.filter_user_aria"><option value="" data-i18n="time_analytics.opt_all_users"><?= htmlspecialchars($t('time_analytics.opt_all_users', 'Все пользователи'), ENT_QUOTES, 'UTF-8') ?></option></select>
      <div class="form-check crm-field-inline"><input class="form-check-input" type="checkbox" id="timeAnalyticsEarningsAmbiguousFilter"><label class="form-check-label" for="timeAnalyticsEarningsAmbiguousFilter" data-i18n="time_analytics.filter_ambiguous"><?= htmlspecialchars($t('time_analytics.filter_ambiguous', 'Только неоднозначные'), ENT_QUOTES, 'UTF-8') ?></label></div>
      <div class="form-check crm-field-inline"><input class="form-check-input" type="checkbox" id="timeAnalyticsEarningsExpandToggle"><label class="form-check-label" for="timeAnalyticsEarningsExpandToggle" data-i18n="time_analytics.toggle_expand"><?= htmlspecialchars($t('time_analytics.toggle_expand', 'Развернуть по клиентам и проектам'), ENT_QUOTES, 'UTF-8') ?></label></div>
    ...
    <span id="timeAnalyticsEarningsPeriodBadge" class="badge crm-badge-soft mb-2 d-none"></span>
    <div class="crm-card crm-section-card p-0 table-responsive"><table class="table table-hover align-middle mb-0 crm-table crm-matrix-table"><thead id="timeAnalyticsEarningsHead"></thead><tbody id="timeAnalyticsEarningsBody"><tr><td colspan="14" class="text-muted" data-i18n="page.loading"><?= htmlspecialchars($t('page.loading', 'Загрузка данных...'), ENT_QUOTES, 'UTF-8') ?></td></tr></tbody></table></div>
