...
], $t('tasks.page_title', 'Задачи'), $t('tasks.subtitle', 'Планирование, контроль сроков и работа с родительскими и дочерними задачами.'), (empty($is_external_user) || $external_role === 'executor' ? '<button class="btn crm-btn-primary" type="button" data-open-modal="createTaskModal">' . htmlspecialchars($t('tasks.btn_create_task', 'Создать задачу'), ENT_QUOTES, 'UTF-8') . '</button>' : '')); ?>
...
<section class="crm-kanban-filters crm-filters-card" aria-label="<?= htmlspecialchars($t('tasks.filters_aria', 'Фильтры задач'), ENT_QUOTES, 'UTF-8') ?>" data-i18n-aria-label="tasks.filters_aria">
...
<section id="tasksStates">
  <div data-state-item="default">
    <div class="table-responsive crm-card p-0" data-tasks-list-view>
      <table class="table table-hover align-middle mb-0 crm-table" data-select-table data-bulk-target="bulkActionsBar"><thead><tr>                        <th style="width:40px"><input class="form-check-input" type="checkbox" data-select-all aria-label="<?= htmlspecialchars($t('tasks.select_all_aria', 'Выбрать все задачи'), ENT_QUOTES, 'UTF-8') ?>" data-i18n-aria-label="tasks.select_all_aria"></th><th style="width:62px"><button type="button" class="btn btn-link p-0 crm-th-sort" data-tasks-sort="task_key" data-i18n="tasks.th_key"><?= htmlspecialchars($t('tasks.th_key', 'Ключ'), ENT_QUOTES, 'UTF-8') ?></button></th><th><button type="button" class="btn btn-link p-0 crm-th-sort" data-tasks-sort="title" data-i18n="tasks.th_task"><?= htmlspecialchars($t('tasks.th_task', 'Задача'), ENT_QUOTES, 'UTF-8') ?></button> / <button type="button" class="btn btn-link p-0 crm-th-sort" data-tasks-sort="project_title" data-i18n="tasks.th_project"><?= htmlspecialchars($t('tasks.th_project', 'Проект'), ENT_QUOTES, 'UTF-8') ?></button></th><th data-i18n="tasks.th_people"><?= htmlspecialchars($t('tasks.th_people', 'Клиент / Менеджер / Исполнитель'), ENT_QUOTES, 'UTF-8') ?></th><th>
<button type="button" class="btn btn-link p-0 crm-th-sort" data-tasks-sort="due_at" data-i18n="tasks.th_due">
<?= htmlspecialchars($t('tasks.th_due', 'Срок'), ENT_QUOTES, 'UTF-8') ?>
</button> / <button type="button" class="btn btn-link p-0 crm-th-sort" data-tasks-sort="status_code" data-i18n="tasks.th_status">
<?= htmlspecialchars($t('tasks.th_status', 'Статус'), ENT_QUOTES, 'UTF-8') ?>
</button> / <button type="button" class="btn btn-link p-0 crm-th-sort" data-tasks-sort="priority_code" data-i18n="tasks.th_priority">
<?= htmlspecialchars($t('tasks.th_priority', 'Приоритет'), ENT_QUOTES, 'UTF-8') ?>
</button>
</th><th style="width:170px" data-i18n="tasks.th_actions"><?= htmlspecialchars($t('tasks.th_actions', 'Действия'), ENT_QUOTES, 'UTF-8') ?></th></tr></thead><tbody>
        <tr><td colspan="6" class="text-muted" data-i18n="tasks.loading"><?= htmlspecialchars($t('tasks.loading', 'Загрузка задач...'), ENT_QUOTES, 'UTF-8') ?></td></tr>
      </tbody></table>
    </div>
  ...
  <div data-state-item="empty" class="d-none"><div class="crm-empty-state"><strong data-i18n="tasks.empty_title"><?= htmlspecialchars($t('tasks.empty_title', 'Задач пока нет'), ENT_QUOTES, 'UTF-8') ?></strong><p class="mb-3" data-i18n="tasks.empty_text"><?= htmlspecialchars($t('tasks.empty_text', 'Создайте первую задачу, чтобы запустить рабочий поток команды.'), ENT_QUOTES, 'UTF-8') ?></p><?php if (empty($is_external_user) || $external_role === 'executor'): ?><button class="btn crm-btn-primary" data-open-modal="createTaskModal" data-i18n="tasks.btn_create_task"><?= htmlspecialchars($t('tasks.btn_create_task', 'Создать задачу'), ENT_QUOTES, 'UTF-8') ?></button><?php endif; ?></div></div>
