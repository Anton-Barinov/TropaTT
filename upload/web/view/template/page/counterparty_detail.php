...
<div class="crm-pr-tabs" role="tablist" aria-label="<?= htmlspecialchars($t('counterparty_detail.tabs_label', 'Разделы контрагента'), ENT_QUOTES, 'UTF-8') ?>">
  ...
  <button class="crm-pr-tab" type="button" role="tab" aria-selected="false" data-cp-tab="projects" id="counterpartyTabProjects"><?= htmlspecialchars($t('counterparty_detail.section_projects', 'Проекты'), ENT_QUOTES, 'UTF-8') ?><span class="crm-pr-tab-count" id="counterpartyProjectsTabCount">0</span></button>
...
<div class="crm-pr-panel" data-cp-panel="projects" role="tabpanel">
  <section class="crm-card crm-section-card">
    <div class="crm-section-head"><div><h2 class="h6 mb-0" data-i18n="counterparty_detail.section_projects"><?= htmlspecialchars($t('counterparty_detail.section_projects', 'Проекты'), ENT_QUOTES, 'UTF-8') ?></h2><div class="crm-section-note" data-i18n="counterparty_detail.section_projects_note"><?= htmlspecialchars($t('counterparty_detail.section_projects_note', 'Проекты, связанные с этим контрагентом.'), ENT_QUOTES, 'UTF-8') ?></div></div><button class="btn btn-sm crm-btn-primary" id="counterpartyProjectCreateBtn" type="button" data-i18n="counterparty_detail.btn_create_project_tab"><?= htmlspecialchars($t('counterparty_detail.btn_create_project_tab', 'Создать проект'), ENT_QUOTES, 'UTF-8') ?></button></div>
    <div class="table-responsive"><table class="table crm-table mb-0"><thead><tr><th data-i18n="counterparty_detail.th_project_name"><?= htmlspecialchars($t('counterparty_detail.th_project_name', 'Проект'), ENT_QUOTES, 'UTF-8') ?></th><th data-i18n="counterparty_detail.th_project_status"><?= htmlspecialchars($t('counterparty_detail.th_project_status', 'Статус'), ENT_QUOTES, 'UTF-8') ?></th><th data-i18n="counterparty_detail.th_project_tasks"><?= htmlspecialchars($t('counterparty_detail.th_project_tasks', 'Задач'), ENT_QUOTES, 'UTF-8') ?></th><th data-i18n="counterparty_detail.th_project_updated"><?= htmlspecialchars($t('counterparty_detail.th_project_updated', 'Обновлён'), ENT_QUOTES, 'UTF-8') ?></th></tr></thead><tbody id="counterpartyDetailProjectsBody"><tr><td colspan="4" class="text-muted" data-i18n="counterparty_detail.loading_projects"><?= htmlspecialchars($t('counterparty_detail.loading_projects', 'Загрузка проектов...'), ENT_QUOTES, 'UTF-8') ?></td></tr></tbody></table></div>
  </section>
</div>
