          ...
          <div class="modal-content">
            ...
            <form id="adminWorkflowCreateForm">
              ...
              <div class="modal-body">
                ...
                <div class="row g-3">
                  <div class="col-md-6">
                    ...
                    <select id="workflowTriggerCode" class="form-select" name="trigger_code" required>
                      ...
                      <option value="worklog_logged" data-i18n="admin_workflow.trigger_worklog_logged"><?= htmlspecialchars($t('admin_workflow.trigger_worklog_logged', 'Записано время по задаче'), ENT_QUOTES, 'UTF-8') ?></option>
                    </select>
                  </div>
                  <div class="col-md-6">
                    ...
                    <select id="workflowActionCode" class="form-select" name="action_code" required>
                      ...
                      <option value="notify_manager" data-i18n="admin_workflow.action_notify_manager"><?= htmlspecialchars($t('admin_workflow.action_notify_manager', 'Уведомить руководителя'), ENT_QUOTES, 'UTF-8') ?></option>
                ...
                <!-- Filter conditions for the time-tracking trigger -->
                <div class="crm-workflow-action-panel mt-3 d-none" data-filter-panel="worklog_logged">
                  <label class="form-label fw-bold" data-i18n="admin_workflow.section_conditions"><?= htmlspecialchars($t('admin_workflow.section_conditions', 'Условия срабатывания'), ENT_QUOTES, 'UTF-8') ?></label>
                  <div class="crm-inline-help mb-2"><i class="fa-solid fa-circle-info" aria-hidden="true"></i><span data-i18n="admin_workflow.help_worklog_conditions"><?= htmlspecialchars($t('admin_workflow.help_worklog_conditions', 'Правило сработает один раз в момент пересечения порога. «Без перерыва» считает непрерывную сессию: после перерыва дольше указанного начинается новая сессия.'), ENT_QUOTES, 'UTF-8') ?></span></div>
                  <div class="row g-3">
                    <div class="col-md-4"><label class="form-label" for="workflowWindowType" data-i18n="admin_workflow.field_window_type"><?= htmlspecialchars($t('admin_workflow.field_window_type', 'Период'), ENT_QUOTES, 'UTF-8') ?></label><select id="workflowWindowType" class="form-select" name="window_type"><option value="day" data-i18n="admin_workflow.opt_window_day"><?= htmlspecialchars($t('admin_workflow.opt_window_day', 'За день'), ENT_QUOTES, 'UTF-8') ?></option><option value="continuous" data-i18n="admin_workflow.opt_window_continuous"><?= htmlspecialchars($t('admin_workflow.opt_window_continuous', 'Без перерыва'), ENT_QUOTES, 'UTF-8') ?></option></select></div>
                    <div class="col-md-4"><label class="form-label" for="workflowThresholdHours" data-i18n="admin_workflow.field_threshold_hours"><?= htmlspecialchars($t('admin_workflow.field_threshold_hours', 'Порог, часов'), ENT_QUOTES, 'UTF-8') ?></label><input id="workflowThresholdHours" class="form-control" type="number" name="threshold_hours" min="0.5" step="0.5" value="8"></div>
                    <div class="col-md-4"><label class="form-label" for="workflowScope" data-i18n="admin_workflow.field_scope"><?= htmlspecialchars($t('admin_workflow.field_scope', 'Считать по'), ENT_QUOTES, 'UTF-8') ?></label><select id="workflowScope" class="form-select" name="scope"><option value="task" data-i18n="admin_workflow.opt_scope_task"><?= htmlspecialchars($t('admin_workflow.opt_scope_task', 'Задаче'), ENT_QUOTES, 'UTF-8') ?></option><option value="user" data-i18n="admin_workflow.opt_scope_user"><?= htmlspecialchars($t('admin_workflow.opt_scope_user', 'Всем задачам пользователя'), ENT_QUOTES, 'UTF-8') ?></option></select></div>
                    <div class="col-md-6"><label class="form-label" for="workflowBreakMinutes" data-i18n="admin_workflow.field_break_minutes"><?= htmlspecialchars($t('admin_workflow.field_break_minutes', 'Перерыв (минут) для режима «без перерыва»'), ENT_QUOTES, 'UTF-8') ?></label><input id="workflowBreakMinutes" class="form-control" type="number" name="break_threshold_minutes" min="1" step="1" value="90"></div>
                    <div class="col-md-6"><label class="form-label" for="workflowConditionUsers" data-i18n="admin_workflow.field_condition_users"><?= htmlspecialchars($t('admin_workflow.field_condition_users', 'Только для пользователей (необязательно)'), ENT_QUOTES, 'UTF-8') ?></label><select id="workflowConditionUsers" class="form-select" name="condition_user_public_ids" multiple></select></div>
                  </div>
                </div>
                ...
                <div class="crm-workflow-action-panel mt-3 d-none" data-action-panel="notify_manager">
                  <div class="crm-inline-help mb-2"><i class="fa-solid fa-circle-info" aria-hidden="true"></i><span data-i18n="admin_workflow.help_manager_auto"><?= htmlspecialchars($t('admin_workflow.help_manager_auto', 'Если получатели не выбраны, система сама найдёт руководителя исполнителя: руководителя команды, затем создателя пользователя, затем менеджера задачи.'), ENT_QUOTES, 'UTF-8') ?></span></div>
                  <div class="row g-3">
                    <div class="col-md-6"><label class="form-label" for="workflowManagerUsers" data-i18n="admin_workflow.field_manager_users"><?= htmlspecialchars($t('admin_workflow.field_manager_users', 'Получатели (необязательно)'), ENT_QUOTES, 'UTF-8') ?></label><select id="workflowManagerUsers" class="form-select" name="manager_user_public_ids" multiple></select></div>
                    <div class="col-md-6"><label class="form-label" for="workflowManagerTitle" data-i18n="admin_workflow.field_manager_title"><?= htmlspecialchars($t('admin_workflow.field_manager_title', 'Заголовок'), ENT_QUOTES, 'UTF-8') ?></label><input id="workflowManagerTitle" class="form-control" name="manager_notification_title" placeholder="<?= htmlspecialchars($t('admin_workflow.placeholder_manager_title', 'Внимание: долгая работа без перерыва'), ENT_QUOTES, 'UTF-8') ?>" data-i18n-placeholder="admin_workflow.placeholder_manager_title"></div>
                  </div>
                  <div class="row g-3 mt-1">
                    <div class="col-md-6"><label class="form-label" for="workflowManagerBody" data-i18n="admin_workflow.field_manager_body"><?= htmlspecialchars($t('admin_workflow.field_manager_body', 'Сообщение'), ENT_QUOTES, 'UTF-8') ?></label><textarea id="workflowManagerBody" class="form-control" name="manager_notification_body" rows="3" placeholder="<?= htmlspecialchars($t('admin_workflow.placeholder_manager_body', '{user} записал {total} минут на задачу «{task}» за {day}.'), ENT_QUOTES, 'UTF-8') ?>" data-i18n-placeholder="admin_workflow.placeholder_manager_body"></textarea></div>
                    <div class="col-md-6"><div class="crm-inline-help mt-4"><i class="fa-solid fa-circle-info" aria-hidden="true"></i><span data-i18n="admin_workflow.help_manager_placeholders"><?= htmlspecialchars($t('admin_workflow.help_manager_placeholders', 'Доступные подстановки: {user}, {task}, {minutes}, {total}, {threshold}, {day}.'), ENT_QUOTES, 'UTF-8') ?></span></div></div>
                  </div>
                </div>
