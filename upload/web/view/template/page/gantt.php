...
<body data-page="gantt" data-protected="1" data-gantt-max-tasks="<?= (int)($gantt_max_tasks ?? 0) ?>"><div class="crm-app"><aside class="crm-sidebar"><div class="crm-brand"><span class="crm-brand-mark"></span> <?= htmlspecialchars($t('app.name', 'TropaTT'), ENT_QUOTES, 'UTF-8') ?></div><nav class="nav flex-column crm-nav"></nav></aside>
...
<section class="crm-gantt-shell crm-card crm-section-card">
  ...
  <div class="crm-gantt-board">
    <div class="crm-gantt-scale"><div class="crm-gantt-scale-label" data-i18n="gantt.scale_label"><?= htmlspecialchars($t('gantt.scale_label', 'Проект / задача'), ENT_QUOTES, 'UTF-8') ?></div><div class="crm-gantt-scale-track" id="ganttScaleTrack"><span data-i18n="gantt.loading_scale"><?= htmlspecialchars($t('gantt.loading_scale', 'Загрузка шкалы...'), ENT_QUOTES, 'UTF-8') ?></span></div>
      <div class="crm-gantt"><div class="crm-gantt-rows"></div><div class="crm-gantt-lanes"></div></div>
    </div>
  </div>
</section>
...
<?= module_position('gantt.content.after', ['route' => $route ?? 'gantt']) ?>
</main></div></div>
