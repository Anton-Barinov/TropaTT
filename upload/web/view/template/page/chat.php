...
(function () {
  ...
  function bindMessageActions(scope) {
    ...
    root.querySelectorAll('[data-copy-message]:not([data-chat-bound])').forEach(function (btn) {
      ...
      btn.addEventListener('click', async function () {
        ...
        try {
          ...
          if (label) {
            label.textContent = window.CRM.i18n.t('chat.msg_copied', 'Скопировано');
          } else {
    ...
    root.querySelectorAll('[data-history-message]:not([data-chat-bound])').forEach(function (btn) {
      btn.setAttribute('data-chat-bound', '1');
      btn.addEventListener('click', function () { showMessageHistory(btn.getAttribute('data-history-message')); });
    });
    ...
    root.querySelectorAll('[data-create-task]:not([data-chat-bound])').forEach(function (btn) {
      btn.setAttribute('data-chat-bound', '1');
      btn.addEventListener('click', function () { createTaskFromMessage(btn.getAttribute('data-create-task')); });
    });
    root.querySelectorAll('[data-chat-more]:not([data-chat-bound])').forEach(function (btn) {
      btn.setAttribute('data-chat-bound', '1');
      btn.addEventListener('click', function (event) {
        event.preventDefault();
        event.stopPropagation();
        closeAllChatMoreMenus(root);
        var menuBox = btn.nextElementSibling;
    ...
    root.querySelectorAll('.crm-chat-more-menu:not([data-chat-bound])').forEach(function (menuBox) {
      menuBox.setAttribute('data-chat-bound', '1');
      menuBox.addEventListener('click', function (event) {
        var item = event.target.closest('[role="menuitem"]');
        if (item) closeAllChatMoreMenus(root);
      });
    });
    root.querySelectorAll('.crm-chat-message-extra [data-chat-more]').forEach(function (btn) {
      var menuBox = btn.nextElementSibling;
      if (menuBox && !menuBox.hidden) {
        menuBox.hidden = true;
        btn.setAttribute('aria-expanded', 'false');
      }
    });
  ...
  function closeAllChatMoreMenus(root) {
    root.querySelectorAll('.crm-chat-message-extra').forEach(function (extra) {
  ...
  async function showMessageHistory(messagePublicId) {
    if (!selectedChatId || !messagePublicId) return;
    var items = [];
    try {
      var env = await request('api/v1/chats/' + encodeURIComponent(selectedChatId) + '/messages/' + encodeURIComponent(messagePublicId) + '/history', { method: 'GET' });
      items = (env.data && env.data.items) || [];
    } catch (error) {}
    var body;
    if (!items.length) {
      body = '<div class="crm-chat-history-empty">' + window.CRM.i18n.t('chat.history_empty', 'Нет записей об изменении.') + '</div>';
    } else {
      body = '<div class="crm-chat-history-list">' + items.map(function (entry) {
    ...
    showInfoModal(window.CRM.i18n.t('chat.history_title', 'История изменений'), body);
  }
  ...
  function createTaskFromMessage(messageId) {
    ...
    if (typeof window.CRM.api.hasPermission === 'function' && !window.CRM.api.hasPermission('task.manage')) {
      showInfoModal(window.CRM.i18n.t('chat.permission_title', 'Нет доступа'), window.CRM.i18n.t('chat.create_task_no_perm', 'Недостаточно прав для создания задачи'));
      return;
    }
    ...
    // Prefill the full global task-create modal with data from the chat and,
    // when this is a project chat, with the linked project and its client.
    ...
    var projectPublicId = (currentChat && currentChat.project_public_id) ? String(currentChat.project_public_id) : '';
    var projectClientPublicId = (currentChat && currentChat.project_client_public_id) ? String(currentChat.project_client_public_id) : '';

    window._taskCreatePrefill = {
      title: defaultTitle,
      description: text ? '<p>' + esc(text).replace(/\n/g, '<br>') + '</p>' : '',
      project_public_id: projectPublicId,
      client_public_id: projectClientPublicId,
      source_type: 'chat',
      source_id: selectedChatId,
      source_url: messageLink,
      source_payload_json: JSON.stringify({
        chat_public_id: selectedChatId,
        chat_title: chatTitleText,
        message_public_id: String(message.public_id || ''),
  ...
  // Deep-link: scroll to a specific message (?id=<chat>&message=<msg>)
  function scrollToMessageOnLoad() {
    var params = new URLSearchParams(window.location.search);
    var targetId = params.get('message') || '';
    if (!targetId) return;
    var box = document.getElementById('msgArea');
    if (!box) return;
    var target = box.querySelector('[data-message-id="' + CSS.escape(targetId) + '"]');
    if (!target) return;
  ...
  // Close any open message more-menu when clicking outside of it.
  document.addEventListener('click', function (event) {
    var extra = event.target.closest ? event.target.closest('.crm-chat-message-extra') : null;
    if (extra) return; // handled by the toggle handler
    var box = document.getElementById('msgArea');
    if (box) closeAllChatMoreMenus(box);
  });
  document.addEventListener('keydown', function (event) {
    if (event.key === 'Escape') {
      var box = document.getElementById('msgArea');
