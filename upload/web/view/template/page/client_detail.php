  ...
  <div class="col-lg-4">
    <div class="crm-card crm-section-card mb-3" id="clientAiCard" data-requires-ai-use="1" data-ai-state="idle">
      ...
      <div class="d-flex flex-wrap gap-2 mb-2">
        ...
        <button class="btn btn-sm crm-btn-secondary" type="button" id="clientAiMeetingPrepBtn" data-i18n="client_detail.ai_meeting_prep_btn"><?= htmlspecialchars($t('client_detail.ai_meeting_prep_btn', 'Подготовка к встрече'), ENT_QUOTES, 'UTF-8') ?></button>
        <button class="btn btn-sm crm-btn-secondary" type="button" id="clientAiDataQualityBtn" data-i18n="client_detail.ai_data_quality_btn"><?= htmlspecialchars($t('client_detail.ai_data_quality_btn', 'Качество данных'), ENT_QUOTES, 'UTF-8') ?></button>
        <button class="btn btn-sm crm-btn-secondary" type="button" id="clientAiSafeReportBtn" data-i18n="client_detail.ai_safe_report_btn"><?= htmlspecialchars($t('client_detail.ai_safe_report_btn', 'Client-safe отчёт'), ENT_QUOTES, 'UTF-8') ?></button>
        <button class="btn btn-sm crm-btn-secondary" type="button" id="clientAiPreviewBtn" disabled data-i18n="client_detail.ai_preview_btn"><?= htmlspecialchars($t('client_detail.ai_preview_btn', 'Предпросмотр'), ENT_QUOTES, 'UTF-8') ?></button>
    ...
    <div class="crm-card crm-section-card mt-3">
      ...
      <div class="mt-3 pt-3 border-top"><h3 class="h6 d-flex align-items-center gap-2 mb-2" data-i18n="client_detail.team_knowledge_title"><span class="crm-icon text-muted" aria-hidden="true"><i class="fa-solid fa-users" aria-hidden="true"></i></span><?= htmlspecialchars($t('client_detail.team_knowledge_title', 'Материалы команды'), ENT_QUOTES, 'UTF-8') ?></h3><div id="clientTeamKnowledgeList"><div class="text-muted small" data-i18n="page.loading"><?= htmlspecialchars($t('page.loading', 'Загрузка...'), ENT_QUOTES, 'UTF-8') ?></div></div></div>
...
(function () {
  ...
  waitForApi(async function () {
    ...
    var teamEl = document.getElementById('clientTeamKnowledgeList');
    if (teamEl && window.CRM && window.CRM.teamMaterials) {
      window.CRM.teamMaterials.render({
        container: teamEl,
        entityType: 'client',
        entityPublicId: clientId,
        api: api,
        texts: {
          ...
          empty: <?= json_encode($t('client_detail.team_knowledge_empty', 'Нет материалов команды'), JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP) ?>,
