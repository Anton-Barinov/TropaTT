...
(function () {
  ...
  waitForApi(async function () {
    ...
    var teamEl = document.getElementById('taskTeamKnowledgeList');
    if (teamEl) {
      try {
        var teamEnvelope = await api.request('api/v1/knowledge/entities/task/' + encodeURIComponent(taskId) + '/team-pages', { method: 'GET' });
        var teamItems = teamEnvelope.data && teamEnvelope.data.items || [];
        if (!teamItems.length) {
          teamEl.innerHTML = '<div class="text-muted small"><?= htmlspecialchars($t('task_detail.team_knowledge_empty', 'Нет материалов команды'), ENT_QUOTES, 'UTF-8') ?></div>';
        } else {
