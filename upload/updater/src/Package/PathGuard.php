...
final class PathGuard
{
    ...
    public function normalize(string $path): ?string
    {
        ...
        // SEC-009: Strip trailing slash to allow directory entries like "lib/"
        // Directory entries are created by most archive tools (zip -r, etc.)
        $path = rtrim($path, '/');
        if ($path === '') {
            return null; // pure "/" or "///" → not allowed
        }
