...
final class AiAuthorTimestampCoverageMigration implements MigrationInterface
{
    ...
    {
        return '20260427_000035_ai_author_timestamp_coverage';
    }

    public function description(): string
    {
        return 'Ensure AI mutable tables have author fields where applicable';
    }

    public function up(PDO $pdo, string $driver): void
    {
        if ($this->tableExists($pdo, $driver, 'ai_prompt_templates')) {
            $this->ensureColumn($pdo, $driver, 'ai_prompt_templates', 'updated_by_user_id', 'INTEGER NULL');
        }

        if ($this->tableExists($pdo, $driver, 'ai_json_schemas')) {
            $this->ensureColumn($pdo, $driver, 'ai_json_schemas', 'updated_by_user_id', 'INTEGER NULL');
        }
    }
