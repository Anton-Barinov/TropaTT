...
final class KnowledgeBaseSeeder
{
    ...
    private function seedTemplates(): void
    {
        ...
        foreach ($templates as $tmpl) {
            try {
                ...
                error_log('[SeedKB::seedTemplates] Template "' . $tmpl['title'] . '": ' . $e->getMessage());
                echo "  ! {$tmpl['title']}: Creation failed. Check server logs for details.\n";
            }
        }
    }
    ...
    private function createSpace(string $title, string $slug, string $description, string $icon, string $color): void
    {
        try {
            ...
            error_log('[SeedKB::createSpace] Space "' . $title . '": ' . $e->getMessage());
            echo "  ✗ Пространство {$title}: Creation failed. Check server logs for details.\n";
        }
    }
    ...
    private function createPage(string $spaceKey, string $title, string $pageType, string $status, string $contentHtml): void
    {
        ...
        try {
            ...
            error_log('[SeedKB::createPage] Page "' . $title . '": ' . $e->getMessage());
                ...
                if (isset($this->pages[$key])) {
                    ...
                    if ($uid > 0) {
                        try {
                            ...
                            error_log('[SeedKB::addFavorites] ' . $e->getMessage());
                        }
                    }
                    $idx++;
                }
            ...
            foreach ($tagCodes as $code) {
                if (isset($this->tags[$code])) {
                    try {
                        ...
                        error_log('[SeedKB::addTagsToPages] Tag "' . $code . '" for page "' . $pageTitle . '": ' . $e->getMessage());
                    }
                }
            }
        }
    ...
    private function linkPage(string $pagePublicId, string $entityType, string $entityPublicId, string $relationType): void
    {
        try {
            ...
            error_log('[SeedKB::linkPage] Link page=' . $pagePublicId . ' entity=' . $entityType . ':' . $entityPublicId . ': ' . $e->getMessage());
        }
    }
    ...
    private function addComment(string $pageTitle, string $body): void
    {
        ...
        try {
            ...
            error_log('[SeedKB::addComment] Page "' . $pageTitle . '": ' . $e->getMessage());
        }
    }
    ...
    private function printSummary(): void
    {
        ...
        try {
            ...
            error_log('[SeedKB::printSummary] ' . $e->getMessage());
            echo "  Ошибка получения статистики. Check server logs for details.\n";
        }
