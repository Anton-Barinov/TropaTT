<?php
return [
    'not_found' => 'Module introuvable',
    'validation_failed' => 'Échec de la validation du module',
    'already_installed' => 'Le module est déjà installé',
    'migration_failed' => 'Échec de la migration',
    'not_installed' => 'Le module n\'est pas installé',
    'manifest_not_found' => 'Manifeste du module introuvable',
    'core_incompatible' => 'Le module nécessite le noyau',
    'installed' => 'Module installé avec succès',
    'activated' => 'Module activé avec succès',
    'deactivated' => 'Module désactivé avec succès',
    'removed' => 'Module supprimé avec succès',
    'installed_from_url' => 'Module installé depuis une URL',
    'installed_from_file' => 'Module installé depuis un fichier',
    'config_updated' => 'Configuration mise à jour',
    'errors_cleared' => 'Erreurs effacées',
    'url_required' => 'L\'URL est requise',
    'url_not_allowed' => 'L\'URL n\'est pas autorisée',
    'file_data_required' => 'Les données du fichier sont requises',
    'invalid_base64' => 'Données base64 invalides',
    'name_required' => 'Le nom du module est requis',
    'bulk_completed' => 'Opération groupée terminée',
    'invalid_name' => 'Nom de module invalide',
    'operation_failed' => 'L’opération sur le module a échoué',
    'install_failed' => 'Impossible d’installer le module. Voir le journal du serveur',
    'activate_failed' => 'Impossible d’activer le module',
    'deactivate_failed' => 'Impossible de désactiver le module',
    'uninstall_failed' => 'Impossible de désinstaller le module',
    'update_failed' => 'Impossible de mettre à jour la configuration du module. Voir le journal du serveur',
    'clear_failed' => 'Impossible d’effacer les erreurs du module. Voir le journal du serveur',
    'purged' => 'Module supprimé physiquement du disque',
    'file_too_large' => 'Le fichier du module est trop volumineux',
    'marketplace_unavailable' => 'Le marketplace de modules est indisponible ou désactivé',
    'marketplace_unreachable' => 'Le marketplace de modules ne répond pas',
    'marketplace_module_unavailable' => 'Impossible de charger les détails du module depuis le marketplace',
    'marketplace_root_required' => 'Seul l’administrateur principal peut installer des modules depuis le marketplace',
    'root_required' => 'Accès administrateur principal requis pour cette opération',
    'marketplace_already_installed' => 'Le module est déjà installé',
    'marketplace_discovered_locally' => 'Le module est déjà présent dans le CRM (par exemple issu du build du noyau) mais n\'est pas installé — utilisez l\'installation locale : /api/v1/modules/{code}/install',
    'marketplace_install_failed' => 'Impossible d’installer le module depuis le marketplace',
    'marketplace_not_installable' => 'Il s’agit d’un produit virtuel du marketplace (par exemple un don), et non d’un module : il ne peut pas être installé dans le CRM',
    'marketplace_installed' => 'Module installé depuis le marketplace',
    'marketplace_activation_failed' => 'Module installé mais non activé — activez-le manuellement',
    'marketplace_package_mismatch' => 'Le paquet du marketplace ne correspond pas au catalogue : son manifeste déclare un autre nom de module. Signalez-le au fournisseur ou publiez une nouvelle version',
    'signing_key_missing' => 'Installation indisponible : MODULE_SIGNING_KEY n’est pas défini dans .env. Les installations depuis le marketplace officiel ne le exigent pas — la clé sert uniquement aux URL directes et aux ZIP téléversés',
    'marketplace_sha256_mismatch' => 'Le paquet téléchargé ne correspond pas au sha256 de la release du marketplace — l’archive est corrompue ou un autre fichier a été servi. Réessayez',
];
