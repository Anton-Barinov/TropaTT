...
MD;
    ...
    private function tools(): array
    {
        ...
        // Each mega-tool covers one domain with an action:enum, replacing
        // dozens of fine-grained CRUD tools. The CRUD tools remain available
        // via the `all` profile for backward compatibility.
        // ═══════════════════════════════════════════════════════════════════
...
$tools[] = $this->tool(
            ...
            [
                'action' => ['type' => 'string', 'enum' => [
                    'list', 'get', 'get_by_key', 'create', 'update', 'delete', 'move', 'bulk_update',
                    'board', 'activity',
                    ...
                    'list_saved_views', 'get_saved_view', 'create_saved_view', 'update_saved_view', 'delete_saved_view', 'archive_saved_view',
                    ...
                    'list_sla_policies', 'create_sla_policy', 'assign_sla', 'get_sla_report',
                    'list_workflow_rules', 'create_workflow_rule',
                    'list_approvals', 'create_approval', 'approve', 'reject',
                ]],
                'public_id' => ['type' => 'string', 'description' => 'Task/subtask/comment/checklist public_id (tsk_...). Required for get/update/delete on specific items.'],
                'task_public_id' => ['type' => 'string', 'description' => 'Parent task public_id for subtasks, comments, checklists, tags, dependencies, relations, estimates.'],
        ...
        // --- crm_project: projects, milestones, cycles, modules, templates, client cabinet ---
        $tools[] = $this->tool(
            'crm_project',
            'Unified project management. Use this for ALL project operations: list, get, create, update, delete, summary, risks, timeline, workload, estimate summaries, milestones, cycles/sprints, project modules, templates, client cabinet. When the user asks about projects, start here. Returns project data with public_id (prj_...), title, status, client.',
            [
                'action' => ['type' => 'string', 'enum' => [
                    'list', 'get', 'create', 'update', 'delete',
                    'summary', 'risks', 'timeline', 'workload', 'estimate_summary', 'milestones_summary',
                    ...
                    'start_cycle', 'complete_cycle', 'reopen_cycle', 'archive_cycle',
                    'get_cycle_summary', 'list_cycle_tasks', 'add_tasks_to_cycle', 'remove_cycle_task',
                    'list_modules', 'get_module', 'create_module', 'update_module', 'archive_module', 'delete_module',
                    'list_templates', 'get_template', 'create_template', 'apply_template', 'delete_template',
                    'list_cabinet_projects', 'get_cabinet_project', 'list_cabinet_tasks',
                ]],
                'public_id' => ['type' => 'string', 'description' => 'Project/milestone/cycle/module/template public_id.'],
                'title' => ['type' => 'string', 'description' => 'Project title. Required for create.'],
                'description' => ['type' => 'string'],
                'status' => ['type' => 'string', 'enum' => ['active', 'new', 'in_progress', 'completed', 'archived']],
                'client_public_id' => ['type' => 'string'],
                'start_date' => ['type' => 'string'],
        ...
        $tools[] = $this->tool(
            'crm_people',
            'Unified people management. Use this for: users (list/get/create/update/delete, tokens, activity), teams, departments, roles/permissions, invitations, impersonation. When the user asks about users, teams, roles, or people, start here. Returns user data with public_id (usr_...), full_name, login.',
            [
                'action' => ['type' => 'string', 'enum' => [
                    'list_users', 'get_user', 'create_user', 'update_user', 'delete_user',
                    'get_user_token', 'rotate_user_token', 'revoke_user_token', 'get_user_activity',
                    ...
                    'list_roles', 'list_permissions', 'get_role_permissions', 'create_role', 'update_role', 'delete_role', 'set_role_permissions',
                    'list_invitations', 'create_invitation',
                    'start_impersonation', 'stop_impersonation', 'get_impersonation_status',
                ]],
                'public_id' => ['type' => 'string', 'description' => 'User/team/department/role public_id.'],
                'email' => ['type' => 'string', 'description' => 'Email for create_user or create_invitation.'],
                'full_name' => ['type' => 'string'],
                'login' => ['type' => 'string'],
                'role_public_ids' => ['type' => 'array', 'items' => ['type' => 'string']],
                'permission_codes' => ['type' => 'array', 'items' => ['type' => 'string']],
        ...
        $tools[] = $this->tool(
            'crm_crm',
            'Unified CRM records management. Use this for: clients, counterparties, companies, contacts, organizations (list/get/create/update/delete + members). When the user asks about clients, contacts, companies, or counterparties, start here. Returns records with public_id (cli_/cpg_/cpy_/cnt_/org_...).',
            [
                'action' => ['type' => 'string', 'enum' => [
                    'list_clients', 'get_client', 'create_client', 'update_client', 'delete_client',
                    'list_counterparties', 'get_counterparty', 'create_counterparty', 'update_counterparty', 'delete_counterparty',
                    ...
                    'list_organization_members', 'add_organization_member', 'remove_organization_member',
                ]],
                'public_id' => ['type' => 'string', 'description' => 'Record public_id (cli_/cpg_/cpy_/cnt_/org_...).'],
                'title' => ['type' => 'string', 'description' => 'Record title/name. Required for create.'],
                'description' => ['type' => 'string'],
                'status' => ['type' => 'string'],
                'user_public_id' => ['type' => 'string', 'description' => 'For organization member operations.'],
        ...
        $tools[] = $this->tool(
            'crm_time',
            'Unified time tracking and calendar. Use this for: worklogs (list/get/create/update/delete, summaries, matrix, earnings, detail), calendar events, business calendars, holidays, working hours. When the user asks about time tracking, calendar, worklogs, or schedule, start here.',
            [
                'action' => ['type' => 'string', 'enum' => [
                    'list_worklogs', 'get_worklog', 'create_worklog', 'update_worklog', 'delete_worklog',
                    'get_worklog_summary', 'get_worklog_matrix', 'get_worklog_detail', 'get_worklog_task_summary', 'get_worklog_earnings',
                    'list_calendar_events', 'get_calendar_event', 'create_calendar_event', 'update_calendar_event', 'delete_calendar_event',
                    'get_calendar_agenda', 'get_calendar_my_month',
                    'list_business_calendars', 'get_business_calendar', 'create_business_calendar', 'update_business_calendar', 'delete_business_calendar',
                ...
                'year' => ['type' => 'integer'],
                'month' => ['type' => 'integer'],
                'hours' => ['type' => 'number', 'description' => 'Hours to log (for create_worklog).'],
                'description' => ['type' => 'string'],
                'limit' => ['type' => 'integer', 'minimum' => 1, 'maximum' => 50, 'default' => 20],
                'page' => ['type' => 'integer', 'minimum' => 1, 'default' => 1],
        ...
        // --- crm_knowledge: spaces, pages, versions, comments, tags, links, files, export/import, AI ---
        $tools[] = $this->tool(
            'crm_knowledge',
            'Unified knowledge base management. Use this for ALL knowledge operations: spaces, pages (CRUD), versions, comments, tags, links, files, favorites, subscriptions, permissions, templates, export/import, AI helpers (summary, explanation, similar, duplicates, orphans, structure). When the user asks about knowledge base, docs, wiki, or articles, start here.',
            [
                'action' => ['type' => 'string', 'enum' => [
                    ...
                    'get_page_draft', 'save_page_draft', 'delete_draft',
                    'favorite_page', 'unfavorite_page', 'list_favorites',
                    'subscribe_page', 'unsubscribe_page',
                    'request_review', 'approve_review', 'reject_review',
                    'list_versions', 'get_version', 'diff_version', 'restore_version',
                    'list_comments', 'add_comment', 'delete_comment', 'resolve_comment', 'reopen_comment',
                    'list_tags', 'attach_tag', 'detach_tag',
                    'list_links', 'link_entity', 'delete_link',
                    'list_files', 'upload_file', 'delete_file',
                    'export_page', 'export_space', 'export_all', 'import_pages',
                    'ai_summary', 'ai_explanation', 'ai_similar', 'ai_checklist', 'ai_faq', 'ai_suggest_for_task',
                    'ai_duplicates', 'ai_orphans', 'ai_structure',
                    'reindex', 'rebuild_permissions', 'cleanup_drafts',
                ]],
        ...
        $tools[] = $this->tool(
            'crm_ai',
            'Unified AI operations. Use this for: executing AI actions (task summary, decomposition, checklist, quality, next action, project summary, risks, dashboard digest, day/week plan, KPI explanation, team workload), managing AI suggestions, providers, models, intents, prompts, schemas, jobs, usage, audit, semantic search. When the user asks for AI analysis, summaries, or plans, start here.',
            [
                'action' => ['type' => 'string', 'enum' => [
                    'execute_action', 'get_availability',
                    'list_suggestions', 'get_suggestion', 'confirm_suggestion', 'dismiss_suggestion', 'preview_suggestion',
                    'task_summary', 'task_next_action', 'task_decomposition', 'task_checklist', 'task_quality',
                    'project_summary', 'project_risks',
                    'dashboard_digest', 'day_plan', 'week_plan',
                    'kpi_explanation', 'risks_explanation', 'team_workload',
                    'list_providers', 'get_provider', 'list_models',
                    'list_intents', 'update_intent',
                    'list_prompts', 'create_prompt', 'update_prompt',
                    'list_schemas', 'create_schema', 'update_schema',
                    'list_jobs', 'get_job', 'run_job', 'dry_run_job', 'retry_job',
                    'list_usage', 'list_audit',
                    'semantic_search',
                    'get_preferences', 'update_preferences',
                ]],
                'public_id' => ['type' => 'string', 'description' => 'Suggestion/job/intent/prompt/schema public_id.'],
                'entity_type' => ['type' => 'string', 'description' => 'Entity type for AI actions (task, project, client, etc.).'],
                'entity_public_id' => ['type' => 'string', 'description' => 'Entity public_id for AI actions.'],
                'intent_code' => ['type' => 'string', 'description' => 'AI intent code (e.g. task_summary, project_risks).'],
        ...
        // --- crm_admin: settings, cache, modules, updates, ops, API clients, webhooks, logs, custom fields, intake, import/export, recycle bin ---
        $tools[] = $this->tool(
            'crm_admin',
            'Unified administration. Use this for: settings, cache, modules, core updates, ops, API clients/keys, webhooks, audit/security/request logs, custom fields, statuses/priorities/tags, intake, import/export, recycle bin, analytics, admin widgets, knowledge admin, role matrix, OpenAPI. When the user asks for admin operations, system config, or maintenance, start here. Requires settings.manage or relevant admin permission.',
            [
                'action' => ['type' => 'string', 'enum' => [
                    'list_settings', 'get_setting',
                    'get_cache_stats', 'clear_cache',
                    'list_modules', 'get_module', 'install_module', 'activate_module', 'deactivate_module', 'uninstall_module', 'get_module_config', 'update_module_config', 'get_module_health',
                    'get_core_version', 'get_core_update_status', 'check_core_update',
                    'get_ops_system', 'get_ops_metrics',
                    'list_api_clients', 'get_api_client', 'create_api_client', 'update_api_client', 'delete_api_client',
                    'list_api_client_keys', 'issue_api_client_key', 'revoke_api_key', 'rotate_api_key', 'get_api_key_usage',
                    'list_webhooks', 'create_webhook', 'update_webhook', 'delete_webhook', 'test_webhook',
                    'list_audit_log', 'list_security_log', 'list_request_logs',
                    'list_custom_fields', 'get_custom_field', 'create_custom_field', 'update_custom_field', 'delete_custom_field',
                    'get_custom_field_values', 'set_custom_field_values',
                    'list_statuses', 'create_status', 'update_status', 'delete_status',
                    'list_priorities', 'create_priority', 'update_priority', 'delete_priority',
                    'list_tags_mgmt', 'create_tag', 'update_tag', 'delete_tag',
                    'list_intake', 'get_intake_item', 'create_intake_item', 'accept_intake_item', 'reject_intake_item',
                    'list_import_jobs', 'create_import_job',
                    'list_export_jobs', 'create_export_job',
                    'list_recycle_bin', 'restore_recycle_bin_item',
                    'get_analytics_summary', 'list_analytics_projects', 'list_analytics_users',
                    'list_feature_flags', 'update_feature_flag',
                    'list_request_logs',
                    'get_openapi_spec', 'list_api_endpoints',
                    'get_admin_summary_widget', 'get_admin_system_widget',
                ]],
                'public_id' => ['type' => 'string', 'description' => 'Record public_id for get/update/delete operations.'],
                'name' => ['type' => 'string', 'description' => 'Setting name, module code, webhook URL, etc.'],
                'value' => ['type' => 'string', 'description' => 'Setting value.'],
                'url' => ['type' => 'string', 'description' => 'Webhook URL (must be HTTPS).'],
                'events' => ['type' => 'array', 'items' => ['type' => 'string']],
                'code' => ['type' => 'string', 'description' => 'Status/priority/tag code.'],
    ...
    private function callTool(array $params): array
    {
        ...
        return match ($name) {
            ...
            // Mega-tools: consolidated intent-based dispatch
            'crm_task' => $this->withPermission('task.manage', fn() => $this->handleMegaTool('crm_task', $arguments)),
            'crm_project' => $this->withPermission('project.manage', fn() => $this->handleMegaTool('crm_project', $arguments)),
            'crm_people' => $this->handleMegaTool('crm_people', $arguments),
            'crm_crm' => $this->handleMegaTool('crm_crm', $arguments),
            'crm_time' => $this->withPermission('task.manage', fn() => $this->handleMegaTool('crm_time', $arguments)),
    ...
    // ═══════════════════════════════════════════════════════════════════
    // MEGA-TOOL DISPATCH: consolidated intent-based routing.
    // Each mega-tool delegates to the existing CRUD handler methods,
    ...
    private function handleMegaTool(string $toolName, array $arguments): array
    {
        ...
        $res = match ($toolName) {
            ...
            'crm_crm' => $this->dispatchCrmMega($action, $arguments),
            'crm_time' => $this->dispatchTimeMega($action, $arguments),
    ...
    private function dispatchTaskMega(string $action, array $args): array
    {
        return $this->megaResult(match ($action) {
            ...
            'create' => $this->crmCreateTask($args),
            'update' => $this->crmUpdateTask($args),
            'delete' => $this->crmDeleteTask($args),
            'move' => $this->crmMoveTask($args),
            'bulk_update' => $this->crmBulkUpdateTasks($args),
            'board' => $this->crmGetTaskBoard($args),
            'activity' => $this->crmListTaskActivity($args),
            'list_comments' => $this->crmListTaskComments($args),
            'add_comment' => $this->crmAddTaskComment($args),
            'update_comment' => $this->crmUpdateComment($args),
            'delete_comment' => $this->crmDeleteComment($args),
            'get_comment_draft' => $this->crmGetCommentDraft($args),
            'save_comment_draft' => $this->crmSaveCommentDraft($args),
            'delete_comment_draft' => $this->crmDeleteCommentDraft($args),
            'list_subtasks' => $this->crmListSubtasks($args),
            'create_subtask' => $this->crmCreateSubtask($args),
            'update_subtask' => $this->crmUpdateSubtask($args),
            'delete_subtask' => $this->crmDeleteSubtask($args),
            'list_tags' => $this->crmListTaskTags($args),
            'attach_tag' => $this->crmAttachTaskTag($args),
            'detach_tag' => $this->crmDetachTaskTag($args),
            ...
            'delete_checklist_item' => $this->crmDeleteChecklistItem($args),
            'list_dependencies' => $this->crmListDependencies($args),
            'create_dependency' => $this->crmCreateDependency($args),
            'delete_dependency' => $this->crmDeleteDependency($args),
            'list_relations' => $this->crmListTaskRelations($args),
            'create_relation' => $this->crmCreateTaskRelation($args),
            'delete_relation' => $this->crmDeleteTaskRelation($args),
            'list_estimates' => $this->crmListTaskEstimates($args),
            'assign_estimate' => $this->crmAssignTaskEstimate($args),
            'remove_estimate' => $this->crmRemoveTaskEstimate($args),
            'list_saved_views' => $this->crmListSavedViews($args),
            'get_saved_view' => $this->crmGetSavedView($args),
            'create_saved_view' => $this->crmCreateSavedView($args),
            'update_saved_view' => $this->crmUpdateSavedView($args),
            'delete_saved_view' => $this->crmDeleteSavedView($args),
            'archive_saved_view' => $this->crmArchiveSavedView($args),
            'list_recurring_rules' => $this->crmListRecurringRules($args),
            'create_recurring_rule' => $this->crmCreateRecurringRule($args),
            'update_recurring_rule' => $this->crmUpdateRecurringRule($args),
            'delete_recurring_rule' => $this->crmDeleteRecurringRule($args),
            'list_reminders' => $this->crmListReminders($args),
            'create_reminder' => $this->crmCreateReminder($args),
            'update_reminder' => $this->crmUpdateReminder($args),
            'delete_reminder' => $this->crmDeleteReminder($args),
            'list_sla_policies' => $this->crmListSlaPolicies($args),
    ...
    private function dispatchProjectMega(string $action, array $args): array
    {
        return $this->megaResult(match ($action) {
            ...
            'update' => $this->crmUpdateProject($args),
            'delete' => $this->crmDeleteProject($args),
            'summary' => $this->crmGetProjectSummary($args),
            'risks' => $this->crmGetProjectRisks($args),
            'timeline' => $this->crmGetProjectTimeline($args),
            'workload' => $this->crmGetProjectWorkload($args),
            'estimate_summary' => $this->crmGetProjectEstimateSummary($args),
            'milestones_summary' => $this->crmGetProjectMilestonesSummary($args),
            'list_milestones' => $this->crmListMilestones($args),
            'get_milestone' => $this->crmGetMilestone($args),
            ...
            'start_cycle' => $this->crmStartCycle($args),
            ...
            'get_module' => $this->crmGetProjectModule($args),
            'create_module' => $this->crmCreateProjectModule($args),
            'update_module' => $this->crmUpdateProjectModule($args),
            'archive_module' => $this->crmArchiveProjectModule($args),
            'delete_module' => $this->crmDeleteProjectModule($args),
            'list_templates' => $this->crmListTemplates($args),
            'get_template' => $this->crmGetTemplate($args),
    ...
    private function dispatchPeopleMega(string $action, array $args): array
    {
        return $this->megaResult(match ($action) {
            ...
            'update_user' => $this->withPermission('user.manage', fn() => $this->crmUpdateUser($args)),
            'delete_user' => $this->withPermission('user.manage', fn() => $this->crmDeleteUser($args)),
            'get_user_token' => $this->withPermission('user.manage', fn() => $this->crmGetUserTokenInfo($args)),
            'rotate_user_token' => $this->withPermission('user.manage', fn() => $this->crmRotateUserToken($args)),
            'revoke_user_token' => $this->withPermission('user.manage', fn() => $this->crmRevokeUserToken($args)),
            'get_user_activity' => $this->withPermission('user.manage', fn() => $this->crmGetUserActivity($args)),
            'list_teams' => $this->toolResult($this->crmListTeams($args)),
            'get_team' => $this->toolResult($this->crmGetTeam($args)),
            'create_team' => $this->withPermission('team.manage', fn() => $this->crmCreateTeam($args)),
            'update_team' => $this->withPermission('team.manage', fn() => $this->crmUpdateTeam($args)),
            'delete_team' => $this->withPermission('team.manage', fn() => $this->crmDeleteTeam($args)),
            'list_departments' => $this->toolResult($this->crmListDepartments($args)),
            'get_department' => $this->toolResult($this->crmGetDepartment($args)),
            'create_department' => $this->withPermission('department.manage', fn() => $this->crmCreateDepartment($args)),
            'update_department' => $this->withPermission('department.manage', fn() => $this->crmUpdateDepartment($args)),
            'delete_department' => $this->withPermission('department.manage', fn() => $this->crmDeleteDepartment($args)),
            'list_roles' => $this->withPermissionAny(['role.view', 'role.manage'], fn() => $this->crmListRoles($args)),
            'list_permissions' => $this->withPermissionAny(['role.view', 'role.manage'], fn() => $this->crmListPermissions()),
            'get_role_permissions' => $this->withPermissionAny(['role.view', 'role.manage'], fn() => $this->crmGetRolePermissions($args)),
            'create_role' => $this->withPermission('role.manage', fn() => $this->crmCreateRole($args)),
            'update_role' => $this->withPermission('role.manage', fn() => $this->crmUpdateRole($args)),
            'delete_role' => $this->withPermission('role.manage', fn() => $this->crmDeleteRole($args)),
            'set_role_permissions' => $this->withPermission('role.manage', fn() => $this->crmSetRolePermissions($args)),
    ...
    private function dispatchCrmMega(string $action, array $args): array
    {
        return $this->megaResult(match ($action) {
            ...
            'update_client' => $this->withPermission('client.manage', fn() => $this->crmUpdateClient($args)),
            'delete_client' => $this->withPermission('client.manage', fn() => $this->crmDeleteClient($args)),
            'list_counterparties' => $this->withPermission('counterparty.manage', fn() => $this->crmListCounterparties($args)),
            'get_counterparty' => $this->withPermission('counterparty.manage', fn() => $this->crmGetCounterparty($args)),
            'create_counterparty' => $this->withPermission('counterparty.manage', fn() => $this->crmCreateCounterparty($args)),
            'update_counterparty' => $this->withPermission('counterparty.manage', fn() => $this->crmUpdateCounterparty($args)),
            'delete_counterparty' => $this->withPermission('counterparty.manage', fn() => $this->crmDeleteCounterparty($args)),
            'list_companies' => $this->withPermission('company.manage', fn() => $this->crmListCompanies($args)),
            'get_company' => $this->withPermission('company.manage', fn() => $this->crmGetCompany($args)),
            'create_company' => $this->withPermission('company.manage', fn() => $this->crmCreateCompany($args)),
            'update_company' => $this->withPermission('company.manage', fn() => $this->crmUpdateCompany($args)),
            'delete_company' => $this->withPermission('company.manage', fn() => $this->crmDeleteCompany($args)),
            'list_contacts' => $this->withPermission('contact.manage', fn() => $this->crmListContacts($args)),
            'get_contact' => $this->withPermission('contact.manage', fn() => $this->crmGetContact($args)),
            'create_contact' => $this->withPermission('contact.manage', fn() => $this->crmCreateContact($args)),
            'update_contact' => $this->withPermission('contact.manage', fn() => $this->crmUpdateContact($args)),
            'delete_contact' => $this->withPermission('contact.manage', fn() => $this->crmDeleteContact($args)),
            'list_organizations' => $this->withPermission('organization.manage', fn() => $this->crmListOrganizations($args)),
            'get_organization' => $this->withPermission('organization.manage', fn() => $this->crmGetOrganization($args)),
            'create_organization' => $this->withPermission('organization.manage', fn() => $this->crmCreateOrganization($args)),
            'update_organization' => $this->withPermission('organization.manage', fn() => $this->crmUpdateOrganization($args)),
            'delete_organization' => $this->withPermission('organization.manage', fn() => $this->crmDeleteOrganization($args)),
    ...
    private function dispatchTimeMega(string $action, array $args): array
    {
        return $this->megaResult(match ($action) {
            ...
            'update_worklog' => $this->crmUpdateWorklog($args),
            'delete_worklog' => $this->crmDeleteWorklog($args),
            'get_worklog_summary' => $this->crmGetWorklogSummary($args),
            'get_worklog_matrix' => $this->crmGetWorklogMatrix($args),
            'get_worklog_detail' => $this->crmGetWorklogDetail($args),
            'get_worklog_task_summary' => $this->crmGetWorklogTaskSummary($args),
            'get_worklog_earnings' => $this->crmGetWorklogEarnings($args),
            'list_calendar_events' => $this->crmListCalendarEvents($args),
            'get_calendar_event' => $this->crmGetCalendarEvent($args),
            'create_calendar_event' => $this->crmCreateCalendarEvent($args),
            'update_calendar_event' => $this->crmUpdateCalendarEvent($args),
    ...
    private function dispatchKnowledgeMega(string $action, array $args): array
    {
        return $this->megaResult(match ($action) {
            ...
            'list_pages' => $this->crmListKnowledgePages($args),
            'get_page' => $this->crmGetKnowledgePage($args),
            'create_page' => $this->crmCreateKnowledgePage($args),
            'update_page' => $this->crmUpdateKnowledgePage($args),
            'archive_page' => $this->crmArchiveKnowledgePage($args),
            'restore_page' => $this->crmRestoreKnowledgePage($args),
            'move_page' => $this->crmMoveKnowledgePage($args),
            'duplicate_page' => $this->crmDuplicateKnowledgePage($args),
            'publish_page' => $this->crmPublishKnowledgePage($args),
            'lock_page' => $this->crmLockKnowledgePage($args),
            'unlock_page' => $this->crmUnlockKnowledgePage($args),
            'get_page_draft' => $this->crmGetKnowledgePageDraft($args),
            'save_page_draft' => $this->crmSaveKnowledgePageDraft($args),
            ...
            'approve_review' => $this->crmApproveKnowledgeReview($args),
            'reject_review' => $this->crmRejectKnowledgeReview($args),
            'list_versions' => $this->crmListKnowledgePageVersions($args),
            'get_version' => $this->crmGetKnowledgePageVersion($args),
            'diff_version' => $this->crmDiffKnowledgePageVersion($args),
            'restore_version' => $this->crmRestoreKnowledgePageVersion($args),
            'list_comments' => $this->crmListKnowledgeComments($args),
            'add_comment' => $this->crmAddKnowledgeComment($args),
            'delete_comment' => $this->crmDeleteKnowledgeComment($args),
            'resolve_comment' => $this->crmResolveKnowledgeComment($args),
            'reopen_comment' => $this->crmReopenKnowledgeComment($args),
            'list_tags' => $this->crmListKnowledgePageTags($args),
            'attach_tag' => $this->crmAttachKnowledgePageTag($args),
            'detach_tag' => $this->crmDetachKnowledgePageTag($args),
            'list_links' => $this->crmListKnowledgePageLinks($args),
            'link_entity' => $this->crmLinkKnowledgePageEntity($args),
            'delete_link' => $this->crmDeleteKnowledgePageLink($args),
            'list_files' => $this->crmListKnowledgeFiles($args),
            'upload_file' => $this->crmUploadKnowledgeFileBase64($args),
    ...
    private function dispatchAiMega(string $action, array $args): array
    {
        return $this->megaResult(match ($action) {
            ...
            'get_suggestion' => $this->withPermission('ai.use', fn() => $this->crmGetAiSuggestion($args)),
            'confirm_suggestion' => $this->withPermission('ai.use', fn() => $this->crmConfirmAiSuggestion($args)),
            'dismiss_suggestion' => $this->withPermission('ai.use', fn() => $this->crmDismissAiSuggestion($args)),
            'preview_suggestion' => $this->withPermission('ai.use', fn() => $this->crmPreviewApplyAiSuggestion($args)),
            'task_summary' => $this->withPermission('task.manage', fn() => $this->crmCreateAiTaskSummary($args)),
            'task_next_action' => $this->withPermission('task.manage', fn() => $this->crmCreateAiTaskNextAction($args)),
            'task_decomposition' => $this->withPermission('task.manage', fn() => $this->crmCreateAiTaskDecomposition($args)),
            'task_checklist' => $this->withPermission('task.manage', fn() => $this->crmCreateAiTaskChecklist($args)),
            'task_quality' => $this->withPermission('task.manage', fn() => $this->crmCreateAiTaskQuality($args)),
            'project_summary' => $this->withPermission('project.manage', fn() => $this->crmCreateAiProjectSummary($args)),
            'project_risks' => $this->withPermission('project.manage', fn() => $this->crmCreateAiProjectRisks($args)),
            'dashboard_digest' => $this->withPermission('ai.use', fn() => $this->crmCreateAiDashboardDigest($args)),
            'day_plan' => $this->withPermission('ai.use', fn() => $this->crmCreateAiMyDayPlan($args)),
            'week_plan' => $this->withPermission('ai.use', fn() => $this->crmCreateAiMyWeekPlan($args)),
            'kpi_explanation' => $this->withPermission('ai.use', fn() => $this->crmCreateAiAnalyticsKpiExplanation($args)),
            'risks_explanation' => $this->withPermission('ai.use', fn() => $this->crmCreateAiAnalyticsRisksExplanation($args)),
            ...
            'create_prompt' => $this->withPermission('settings.manage', fn() => $this->crmCreateAiPrompt($args)),
            'update_prompt' => $this->withPermission('settings.manage', fn() => $this->crmUpdateAiPrompt($args)),
            'list_schemas' => $this->withPermission('settings.manage', fn() => $this->crmListAiJsonSchemas($args)),
            'create_schema' => $this->withPermission('settings.manage', fn() => $this->crmCreateAiJsonSchema($args)),
            'update_schema' => $this->withPermission('settings.manage', fn() => $this->crmUpdateAiJsonSchema($args)),
            'list_jobs' => $this->withPermission('settings.manage', fn() => $this->crmListAiJobs($args)),
            'get_job' => $this->withPermission('settings.manage', fn() => $this->crmGetAiJob($args)),
            'run_job' => $this->withPermission('settings.manage', fn() => $this->crmRunOnceAiJob($args)),
            'dry_run_job' => $this->withPermission('settings.manage', fn() => $this->crmDryRunAiJob($args)),
            'retry_job' => $this->withPermission('settings.manage', fn() => $this->crmRetryAiJob($args)),
            'list_usage' => $this->withPermission('settings.manage', fn() => $this->crmListAiUsage($args)),
            'list_audit' => $this->withPermission('settings.manage', fn() => $this->crmListAiAudit($args)),
    ...
    private function dispatchAdminMega(string $action, array $args): array
    {
        return $this->megaResult(match ($action) {
            ...
            'clear_cache' => $this->withPermission('settings.manage', fn() => $this->crmClearCache()),
            'list_modules' => $this->withPermission('settings.manage', fn() => $this->crmListModules()),
            'get_module' => $this->withPermission('settings.manage', fn() => $this->crmGetModule($args)),
            'install_module' => $this->withPermission('settings.manage', fn() => $this->crmInstallModule($args)),
            'activate_module' => $this->withPermission('settings.manage', fn() => $this->crmActivateModule($args)),
            'deactivate_module' => $this->withPermission('settings.manage', fn() => $this->crmDeactivateModule($args)),
            'uninstall_module' => $this->withPermission('settings.manage', fn() => $this->crmUninstallModule($args)),
            'get_module_config' => $this->withPermission('settings.manage', fn() => $this->crmGetModuleConfig($args)),
            'update_module_config' => $this->withPermission('settings.manage', fn() => $this->crmUpdateModuleConfig($args)),
            'get_module_health' => $this->withPermission('settings.manage', fn() => $this->crmGetModuleHealth($args)),
            'get_core_version' => $this->withPermission('settings.manage', fn() => $this->crmGetCoreVersion()),
            'get_core_update_status' => $this->withPermission('settings.manage', fn() => $this->crmGetCoreUpdateStatus()),
            'check_core_update' => $this->withPermission('settings.manage', fn() => $this->crmCheckCoreUpdate()),
            'get_ops_system' => $this->withPermission('settings.manage', fn() => $this->crmGetOpsSystem()),
            'get_ops_metrics' => $this->withPermission('settings.manage', fn() => $this->crmGetOpsMetrics()),
            'list_api_clients' => $this->withPermissionAny(['api_client.view', 'api_client.manage'], fn() => $this->crmListApiClients($args)),
            'get_api_client' => $this->withPermissionAny(['api_client.view', 'api_client.manage'], fn() => $this->crmGetApiClient($args)),
            'create_api_client' => $this->withPermission('api_client.manage', fn() => $this->crmCreateApiClient($args)),
            'update_api_client' => $this->withPermission('api_client.manage', fn() => $this->crmUpdateApiClient($args)),
            'delete_api_client' => $this->withPermission('api_client.manage', fn() => $this->crmDeleteApiClient($args)),
            'list_api_client_keys' => $this->withPermissionAny(['api_client.view', 'api_client.manage'], fn() => $this->crmListApiClientKeys($args)),
            'issue_api_client_key' => $this->withPermission('api_client.manage', fn() => $this->crmIssueApiClientKey($args)),
            'revoke_api_key' => $this->withPermission('api_client.manage', fn() => $this->crmRevokeApiKey($args)),
            'rotate_api_key' => $this->withPermission('api_client.manage', fn() => $this->crmRotateApiKey($args)),
            'get_api_key_usage' => $this->withPermissionAny(['api_client.view', 'api_client.manage'], fn() => $this->crmGetApiKeyUsage($args)),
            'list_webhooks' => $this->withPermission('webhook.manage', fn() => $this->crmListWebhooks($args)),
            'create_webhook' => $this->withPermission('webhook.manage', fn() => $this->crmCreateWebhook($args)),
            'update_webhook' => $this->withPermission('webhook.manage', fn() => $this->crmUpdateWebhook($args)),
            'delete_webhook' => $this->withPermission('webhook.manage', fn() => $this->crmDeleteWebhook($args)),
            'test_webhook' => $this->withPermission('webhook.manage', fn() => $this->crmTestWebhook($args)),
            'list_audit_log' => $this->withPermissionAny(['logs.view', 'settings.manage'], fn() => $this->crmListAuditLog($args)),
            'list_security_log' => $this->withPermissionAny(['logs.view', 'settings.manage'], fn() => $this->crmListSecurityLog($args)),
            'list_request_logs' => $this->withPermissionAny(['logs.view', 'settings.manage'], fn() => $this->crmListRequestLogs($args)),
            'list_custom_fields' => $this->withPermission('task.manage', fn() => $this->crmListCustomFields($args)),
            'get_custom_field' => $this->withPermission('task.manage', fn() => $this->crmGetCustomField($args)),
            'create_custom_field' => $this->withPermission('task.manage', fn() => $this->crmCreateCustomField($args)),
            'update_custom_field' => $this->withPermission('task.manage', fn() => $this->crmUpdateCustomField($args)),
            'delete_custom_field' => $this->withPermission('task.manage', fn() => $this->crmDeleteCustomField($args)),
            'get_custom_field_values' => $this->withPermission('task.manage', fn() => $this->crmGetCustomFieldValues($args)),
            'set_custom_field_values' => $this->withPermission('task.manage', fn() => $this->crmSetCustomFieldValues($args)),
            'list_statuses' => $this->toolResult($this->crmListStatuses($args)),
            'create_status' => $this->withPermission('task.manage', fn() => $this->crmCreateStatus($args)),
            'update_status' => $this->withPermission('task.manage', fn() => $this->crmUpdateStatus($args)),
            'delete_status' => $this->withPermission('task.manage', fn() => $this->crmDeleteStatus($args)),
            'list_priorities' => $this->toolResult($this->crmListPriorities($args)),
            'create_priority' => $this->withPermission('task.manage', fn() => $this->crmCreatePriority($args)),
            'update_priority' => $this->withPermission('task.manage', fn() => $this->crmUpdatePriority($args)),
            'delete_priority' => $this->withPermission('task.manage', fn() => $this->crmDeletePriority($args)),
            'list_tags_mgmt' => $this->toolResult($this->crmListTags($args)),
            'create_tag' => $this->withPermission('task.manage', fn() => $this->crmCreateTag($args)),
            'update_tag' => $this->withPermission('task.manage', fn() => $this->crmUpdateTag($args)),
            'delete_tag' => $this->withPermission('task.manage', fn() => $this->crmDeleteTag($args)),
            'list_intake' => $this->withPermissionAny(['intake.view', 'intake.manage'], fn() => $this->crmListIntakeItems($args)),
            'get_intake_item' => $this->withPermissionAny(['intake.view', 'intake.manage'], fn() => $this->crmGetIntakeItem($args)),
            'create_intake_item' => $this->withPermissionAny(['intake.create', 'intake.manage'], fn() => $this->crmCreateIntakeItem($args)),
            'accept_intake_item' => $this->withPermissionAny(['intake.accept', 'intake.manage'], fn() => $this->crmAcceptIntakeItem($args)),
            'reject_intake_item' => $this->withPermissionAny(['intake.manage'], fn() => $this->crmRejectIntakeItem($args)),
            'list_import_jobs' => $this->withPermission('import.manage', fn() => $this->crmListImportJobs($args)),
            'create_import_job' => $this->withPermission('import.manage', fn() => $this->crmCreateImportJob($args)),
            'list_export_jobs' => $this->withPermission('export.manage', fn() => $this->crmListExportJobs($args)),
            'create_export_job' => $this->withPermission('export.manage', fn() => $this->crmCreateExportJob($args)),
            'list_recycle_bin' => $this->withPermission('recycle_bin.manage', fn() => $this->crmListRecycleBin($args)),
            'restore_recycle_bin_item' => $this->withPermission('recycle_bin.manage', fn() => $this->crmRestoreRecycleBinItem($args)),
            'get_analytics_summary' => $this->withPermissionAny(['analytics.view', 'task.manage'], fn() => $this->crmGetAnalyticsSummary()),
            'list_analytics_projects' => $this->withPermissionAny(['analytics.view', 'task.manage'], fn() => $this->crmListAnalyticsProjects($args)),
            'list_analytics_users' => $this->withPermissionAny(['analytics.view', 'task.manage'], fn() => $this->crmListAnalyticsUsers($args)),
            'list_feature_flags' => $this->withPermission('settings.manage', fn() => $this->crmListFeatureFlags($args)),
            'update_feature_flag' => $this->withPermission('settings.manage', fn() => $this->crmUpdateFeatureFlag($args)),
            'get_openapi_spec' => $this->withPermission('settings.manage', fn() => $this->crmGetOpenapiSpec()),
    ...
    private function crmListModules(): array
    {
        ...
        foreach ($pluginManager->getDiscovered() as $name => $manifest) {
            ...
            $items[] = [
                ...
                'author_url' => (string)($manifest->authorUrl ?? ''),
    ...
    private function filters(array $arguments, int $defaultLimit, int $maxLimit): array
    {
        $filters = $this->pick($arguments, [
            ...
            'space_public_id', 'sort', 'order', 'include_archived_projects',
        ]);
