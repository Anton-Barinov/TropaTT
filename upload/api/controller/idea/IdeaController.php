...
final class IdeaController extends BaseController
{
    ...
    public function create(): JsonResponse
    {
        ...
        try {
            ...
            AppLog::error('[IdeaController::create][' . $publicId . '] Notification failed: ' . $e->getMessage());
        }
    ...
    public function update(array $params = []): JsonResponse
    {
        ...
        $pdo = $this->container->get('db.pdo');
        $stmt = $pdo->prepare("SELECT id, public_id, title, description, author_user_id, category, region, visibility, target_date, created_at, status, vote_count, coverage_json, known_facts_json, ai_analysis_at FROM ideas WHERE public_id = :pid AND (:organization_id_filter IS NULL OR organization_id = :organization_id)");
        $stmt->execute(['pid' => $publicId, 'organization_id_filter' => $this->activeOrganizationId(), 'organization_id' => $this->activeOrganizationId()]);
    ...
    public function vote(array $params = []): JsonResponse
    {
        ...
        if ($existing->fetchColumn()) {
            ...
            try {
                ...
                AppLog::error('[IdeaController::vote][' . $publicId . '] Notification failed: ' . $e->getMessage());
            }
        }
    ...
    public function updateStatus(array $params = []): JsonResponse
    {
        ...
        try {
            ...
            AppLog::error('[IdeaController::updateStatus][' . $publicId . '] Notification failed: ' . $e->getMessage());
        }
    ...
    public function aiCreateTasks(array $params = []): JsonResponse
    {
        ...
        try {
            ...
            // Log through the application logger: AppLog::error() output is discarded
...
PROMPT;
    ...
    // NOTE: SELECT * is intentional — table name is dynamic (caller-controlled),
    // but sensitive columns are explicitly unset below before returning.
            $stmt->execute(['iid' => $ideaId]);
            $row = $stmt->fetch(\PDO::FETCH_ASSOC);
    ...
    public function resetAnalysis(array $params = []): JsonResponse
    {
        ...
        try {
            ...
            try {
                ...
                AppLog::error('[IdeaController] idea_analyses archive skipped: ' . $e->getMessage());
    ...
    public function decomposeTasks(array $params = []): JsonResponse
    {
        ...
        try {
            ...
            AppLog::error('[IdeaController::decomposeTasks] ' . $e->getMessage());
