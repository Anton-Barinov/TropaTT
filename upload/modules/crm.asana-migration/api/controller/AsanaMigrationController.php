...
final class AsanaMigrationController
{
    ...
    public function retryFailed(array $params): JsonResponse { $result = $this->job((string)$params['public_id']); if ($result instanceof JsonResponse) return $result; if (!in_array((string)($result['status'] ?? ''), ['completed_with_warnings', 'failed', 'cancelled'], true)) return JsonResponse::error('INVALID_JOB_STATUS', 'Only a finished job can be retried', 409); if (!$this->repo->requestStatus((string)$params['public_id'], 'queued')) return JsonResponse::error('INVALID_JOB_STATUS', 'Job changed concurrently; retry it again', 409); $count = $this->repo->resetFailedItems((string)$params['public_id']); return JsonResponse::success('ASANA_JOB_RETRY_QUEUED', 'Failed items queued for retry', ['reset_items' => $count]); }
