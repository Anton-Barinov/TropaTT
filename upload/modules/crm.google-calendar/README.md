...
2. **Каждый пользователь задаёт свои креды** на странице модуля (блок «OAuth-креды Google (личные)»): Client ID и Client secret. Они хранятся в БД зашифрованными (`google_calendar_credentials`, `EncryptionService`) и не выводятся через API — страница показывает только маскированный Client ID. Разные пользователи могут использовать разные OAuth-клиенты.
3. Дополнительно можно задать общие креды в `.env` как fallback (используются, пока пользователь не указал свои):
...
В Google Cloud Console зарегистрируйте фактический HTTPS callback вашей установки. Домен не зашит в коде: по умолчанию он берётся из текущего запроса и `SCRIPT_NAME`, поэтому работают домен, поддомен и установка в подпапке. `GOOGLE_REDIRECT_URI` нужен только если внешний reverse-proxy публикует CRM по другому каноническому адресу; альтернативно можно задать `CRM_PUBLIC_URL` без фиксированного домена в коде. `APP_SECRET` обязателен: креды OAuth и refresh/access tokens хранятся только зашифрованными.
