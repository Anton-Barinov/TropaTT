(() => {
  ...
  const render = data => {
    ...
    $('ycalDisconnect').addEventListener('click', () => { if (!window.confirm(window.CRM.i18n.t('yandex_calendar.confirm_disconnect', 'Отключить Яндекс.Календарь и удалить его события из CRM?'))) return; api(`/connections/${encodeURIComponent(connection.public_id)}`, {method:'DELETE'}).then(() => { message(window.CRM.i18n.t('yandex_calendar.disconnected', 'Яндекс.Календарь отключён.')); load(); }).catch(e => message(e.message, true)); });
