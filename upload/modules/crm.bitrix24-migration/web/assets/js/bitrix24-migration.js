(() => {
  ...
  const loadMappings = async () => {
    ...
    try {
      const data = await api(`/connections/${encodeURIComponent(state.selected.public_id)}/user-mappings`);
  ...
  const saveMapping = async mappingId => {
    ...
    try {
      await api(`/connections/${encodeURIComponent(state.selected.public_id)}/user-mappings/${encodeURIComponent(mappingId)}`, {method: 'PATCH', body: JSON.stringify({crm_user_public_id: input.value.trim() || null})});
  ...
  const connectionAction = async (action, id) => {
    ...
    try {
      if (action === 'test') {
        await api(`/connections/${encodeURIComponent(id)}/test`, {method: 'POST', body: '{}'});
        message('Подключение успешно проверено.');
      } else if (action === 'discover') {
        await api(`/connections/${encodeURIComponent(id)}/discover`, {method: 'POST', body: JSON.stringify({entities: ['user']})});
  ...
  const createConnection = async event => {
    ...
    try {
      await api('/connections', {method: 'POST', body: JSON.stringify(input)});
