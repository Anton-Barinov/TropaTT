...
final class Bitrix24Crawler
{
    ...
    public function crawl(array $job, ?callable $heartbeat = null): array
    {
        ...
        $includeComments=(bool)($options['include_comments']??$scope['include_comments']??true);$includeFiles=(bool)($options['include_files']??$scope['include_files']??false);$includeProducts=(bool)($options['include_products']??$scope['include_products']??false);
        $stats=['entities'=>[],'comments'=>0,'files'=>0,'product_rows'=>0,'warnings'=>[]];
        ...
        $collections=[
            'department'=>fn():array=>$this->client->departments(),'user'=>fn():array=>$this->client->users(),'company'=>fn():array=>$this->client->companies(),'contact'=>fn():array=>$this->client->contacts(),'lead'=>fn():array=>$this->client->leads(),'deal'=>fn():array=>$this->client->deals(),'invoice'=>fn():array=>$this->client->invoices(),'quote'=>fn():array=>$this->client->quotes(),'product'=>fn():array=>$this->client->products(),'project'=>fn():array=>$this->client->projects(),'task'=>fn():array=>$this->client->tasks(),'activity'=>fn():array=>$this->client->activities(),'event'=>fn():array=>$this->client->events(),'file'=>fn():array=>$this->client->files(),
        ];
        foreach($types as $type){
            ...
            try{$rows=$collections[$type]();$stats['entities'][$type]=count($rows);foreach($rows as $row){$this->store($job,$type,$row);$id=$this->sourceId($row);if($id==='' )continue;
    ...
    private function store(array $job,string $type,array $row,?string $forcedId=null,?string $parent=null): void
    {
        ...
        if($type==='task'){$parentId=(string)($row['PARENT_ID']??$row['parentId']??'');$type=$parentId!==''?'subtask':'task';$row['_source_parent_id']=$parentId!==''?$parentId:null;}
        $this->repo->upsertItem((int)$job['id'],$type,$id,['source_parent_id'=>$row['_source_parent_id']??null,'status'=>'pending','checksum'=>$this->checksum($row),'source_updated_at'=>$this->date($row['DATE_MODIFY']??$row['CHANGED_DATE']??$row['dateModify']??''),'payload_json'=>$row]);
    }
    ...
    {
        foreach((array)($comment['FILES']??$comment['files']??[]) as $key=>$file){if(!is_array($file))continue;$file['_source_parent_id']=(string)($comment['ID']??$comment['id']??'');$file['_download_url']=(string)($file['urlDownload']??$file['url']??$file['download_url']??'');$this->store($job,'file',$file,(string)($file['id']??$file['ID']??$comment['ID'].'-file-'.$key),(string)($comment['ID']??$comment['id']??''));$stats['files']++;}
    }
