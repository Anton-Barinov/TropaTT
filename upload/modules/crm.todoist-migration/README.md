...
- Основной транспорт — Todoist REST API v2: `https://api.todoist.com/rest/v2`.
...
3. Все source entities сначала попадают в `job_items`; импорт идёт batch-ами по 100 записей в порядке project → section → label → task → comment → attachment.
...
7. Rollback удаляет только объекты с `created_by_job=1`, а изменения описания существующего проекта восстанавливает из сохранённого snapshot; всё выполняется в обратном порядке. Просроченный rollback lease можно безопасно продолжить.

### Пагинация, rate limits и ошибки

Todoist endpoints могут возвращать прямой массив REST v2 либо объект с `results`, `items`, `next_cursor`/`next_offset`. Client поддерживает оба envelope-вида, cursor и offset, ограничивает число страниц защитным пределом и обрабатывает remote collections потоково. HTTP 429 учитывается один раз, использует `Retry-After` и exponential backoff только между попытками; 401/403/404 превращаются в типизированные ошибки.

### Архивы и ограничения

- По умолчанию импортируются активные проекты/tasks. `include_archived` и `include_completed` включаются явно.
- REST v2 не является источником полной истории удалённых задач; completed history ограничена возможностями Sync endpoint и указанным диапазоном дат.
