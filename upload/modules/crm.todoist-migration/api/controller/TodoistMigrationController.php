...
final class TodoistMigrationController
{
    ...
    public function discover(array $p):JsonResponse{$r=$this->connection((string)($p['public_id']??''));if($r instanceof JsonResponse)return$r;$t=EncryptionService::decrypt((string)$r['access_token_encrypted']);if($t===null)return JsonResponse::error('TODOIST_CREDENTIAL_DECRYPT_FAILED','Could not decrypt credentials',500);try{$c=new TodoistClient($this->repo);$c->setConnectionId((int)$r['id']);return JsonResponse::success('TODOIST_DISCOVERY_COMPLETE','Todoist projects discovered',['projects'=>$c->projects($t,true),'user_mappings'=>$this->repo->listUserMappings((int)$r['id'])]);}catch(\Throwable){return JsonResponse::error('TODOIST_DISCOVERY_FAILED','Could not load Todoist projects',400);}}
    ...
    public function startJob(array $p):JsonResponse{return$this->changeJob((string)$p['public_id'],'queued','TODOIST_JOB_QUEUED');}public function pauseJob(array $p):JsonResponse{return$this->changeJob((string)$p['public_id'],'pausing','TODOIST_JOB_PAUSING');}public function resumeJob(array $p):JsonResponse{return$this->changeJob((string)$p['public_id'],'queued','TODOIST_JOB_RESUMED');}public function cancelJob(array $p):JsonResponse{return$this->changeJob((string)$p['public_id'],'cancelling','TODOIST_JOB_CANCELLING');}
