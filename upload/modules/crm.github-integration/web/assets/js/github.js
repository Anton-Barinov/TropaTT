(function () {
    ...
    const API_PREFIX = '_module/crm.github-integration';
    let state = { projects: [] };

    function isPage() {
        return Boolean(document.body && document.body.dataset && document.body.dataset.page === 'module-github-integration');
    }
    ...
    function openConnectionModal() {
        ...
        document.getElementById('connBaseUrl').value = 'https://api.github.com';
    ...
    function loadLinks() {
        ...
        api('/links', 'GET').then(function (data) {
            ...
            if (!links.length) {
                container.innerHTML = '<div class="text-muted py-3">Связанных репозиториев пока нет.</div>';
                return;
            }
            container.innerHTML = '<div class="table-responsive"><table class="table table-sm align-middle">' +
                '<thead><tr><th>Репозиторий</th><th>Проект</th><th>Синхронизация</th><th class="text-end">Действия</th></tr></thead><tbody>' +
                links.map(function (l) {
                    ...
                    return '<tr>' +
                        '<td><strong>' + esc(l.owner) + '/' + esc(l.repo) + '</strong>' + (l.is_dirty === '1' || l.is_dirty === 1 ? ' <span class="badge bg-warning text-dark">ожидает</span>' : '') + '</td>' +
    ...
    function renderProjects() {
        const select = document.getElementById('linkProject');
