...
final class KaitenTargetWriter
{
    ...
    public function reconcileParent(array $job,array $item,array $actor): array
    {
        $source=(string)($item['source_id']??'');$workspace=(string)$job['workspace_gid'];$parent=$this->parent((array)($item['payload']??[]));if($source===''||$parent==='')return $this->result('task',(string)($item['target_public_id']??''),'skipped');$parentMap=$this->repo->findMapping((int)$job['connection_id'],$workspace,'card',$parent);if(!$parentMap||empty($parentMap['target_public_id']))return $this->result('task',(string)($item['target_public_id']??''),'warning',['Parent card mapping is still unavailable.']);$target=(string)($item['target_public_id']??'');if($target==='')return $this->result('task','', 'skipped');$updated=$this->service('service.task')->update($target,['parent_task_public_id'=>(string)$parentMap['target_public_id']],(int)($actor['id']??0),$actor);if(!is_array($updated))return $this->result('task',$target,'warning',['Parent card relation could not be reconciled.']);return $this->result('task',$target,'updated');
    }
