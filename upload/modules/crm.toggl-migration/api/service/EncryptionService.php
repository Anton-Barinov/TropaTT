...
final class EncryptionService
{
    private static function key(): string
    {
        ...
        if ($secret === '') {
            throw new RuntimeException('APP_SECRET is not configured for Toggl credential encryption');
        }
        return hash_hkdf('sha256', $secret, 32, 'toggl-migration');
    }
    ...
    public static function encrypt(string $plaintext): string
    {
        ...
        if (!is_string($ciphertext) || $ciphertext === '') {
            throw new RuntimeException('Failed to encrypt Toggl credential');
        }
