...
- Core-коллекции читаются страницами `page/per_page`; Reports API продолжает выдачу по `X-Next-ID` и `X-Next-Row-Number`.
...
- Running entries (`duration < 0` или без `stop`) не превращаются в ложные worklogs: они помечаются skipped с предупреждением.
...
Импорт выполняется в порядке client → project → tag → task → time_entry. Job имеет lease, checkpoint и пакетную обработку; падение worker повторяет только незавершённый пакет. Не сопоставленный пользователь не подменяется оператором: запись времени получает failed/unresolved, чтобы не исказить отчётность.

Rollback удаляет только targets с `created_by_job=1`. Worklogs удаляются через штатный `WorklogService`, поэтому сохраняются его проверки доступа и аудит; заранее существующие CRM записи не затрагиваются. Неудачные удаления остаются retryable и попадают в лог.
