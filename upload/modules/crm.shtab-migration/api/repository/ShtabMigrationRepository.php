...
final class ShtabMigrationRepository
{
    ...
    public function importItemsBatch(int $job,int $priority=0,int $lastId=0,int $limit=100): array { $sql="SELECT * FROM (SELECT i.*,CASE i.source_type WHEN 'workspace' THEN 10 WHEN 'organization' THEN 10 WHEN 'team' THEN 10 WHEN 'project' THEN 20 WHEN 'tag' THEN 30 WHEN 'user' THEN 35 WHEN 'contact' THEN 40 WHEN 'task' THEN 50 WHEN 'subtask' THEN 55 WHEN 'comment' THEN 60 WHEN 'file' THEN 70 ELSE 80 END AS import_priority FROM module_shtab_job_items i WHERE i.job_id=:j AND i.status='pending') x WHERE (x.import_priority>:p_gt OR (x.import_priority=:p_eq AND x.id>:i)) ORDER BY x.import_priority ASC,x.id ASC LIMIT ".max(1,min(500,$limit));$stmt=$this->pdo->prepare($sql);$stmt->execute(['j'=>$job,'p_gt'=>$priority,'p_eq'=>$priority,'i'=>$lastId]);return$stmt->fetchAll(PDO::FETCH_ASSOC)?:[]; }
