(function () {
  ...
  function authorizeOAuth() { var form = new FormData($('#clickupConnectionForm')); request('/oauth/authorize-url', { method: 'POST', body: { client_id: form.get('client_id'), redirect_uri: form.get('redirect_uri') } }).then(function (envelope) { var result = data(envelope); if (result.authorization_url) window.open(result.authorization_url, '_blank', 'noopener'); }).catch(function () { var error=$('#clickupConnectionError'); error.textContent='Не удалось сформировать OAuth URL.'; error.classList.remove('d-none'); }); }
  document.addEventListener('DOMContentLoaded', function () { $('#clickupAddConnection')?.addEventListener('click', showModal); $('#clickupConnectionForm')?.addEventListener('submit', submitConnection); $('#clickupOAuthAuthorize')?.addEventListener('click', authorizeOAuth); $('#clickupAuthType')?.addEventListener('change', function (event) { var oauth=event.target.value==='oauth2'; $('#clickupPatFields')?.classList.toggle('d-none',oauth); $('#clickupOauthFields')?.classList.toggle('d-none',!oauth); }); $('#clickupDiscover')?.addEventListener('click', discover); $('#clickupStart')?.addEventListener('click', createJob); $('#clickupRefreshJobs')?.addEventListener('click', loadJobs); loadConnections(); loadJobs(); });
})();
