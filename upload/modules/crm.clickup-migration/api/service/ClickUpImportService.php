...
final class ClickUpImportService
{
    public function __construct(
        private readonly ClickUpMigrationRepository $repo,
        private readonly ClickUpClient $client,
        private readonly ClickUpCrawler $crawler,
        private readonly ClickUpTargetWriter $writer
    ) {
    }
    ...
    public function processJob(string $jobPublicId, ?string $leaseToken = null): void
    {
        ...
        if (!$connection) throw new RuntimeException('CLICKUP_CONNECTION_NOT_FOUND');
        $token = EncryptionService::decrypt((string)($connection['access_token_encrypted'] ?? ''));
        if ($token === null) throw new RuntimeException('CLICKUP_CREDENTIAL_DECRYPT_FAILED');
        $this->client->setConnectionId((int)$connection['id']);
        $this->client->setAuthType((string)($connection['auth_type'] ?? 'pat'));
        ...
        if (($existing['phase'] ?? '') === 'crawl' && !empty($existing['after_space_id'])) {
            $job['source_scope']['_after_space_id'] = (string)$existing['after_space_id'];
        }

        $crawl = $resumeImport ? ['resumed' => true, 'warnings' => [], 'crawl_complete' => true] : null;
        if ($crawl === null) {
            $this->repo->updateProgress($jobPublicId, 'crawl', 0, ['message' => 'Loading ClickUp source graph'], $leaseToken);
            ...
            if ($leaseToken !== null && !$this->repo->heartbeat($jobPublicId, $leaseToken)) throw new RuntimeException('CLICKUP_JOB_LEASE_LOST');
            $this->repo->addLog((int)$job['id'], 'info', 'crawl', 'ClickUp source graph batch loaded.', $crawl);

            if (($job['mode'] ?? 'import') !== 'dry_run' && ($crawl['crawl_complete'] ?? true) === false) {
                $after = (string)($crawl['last_space_id'] ?? '');
                if ($after === '') throw new RuntimeException('CLICKUP_CRAWL_CHECKPOINT_FAILED');
                $this->repo->updateCursor($jobPublicId, json_encode(['phase' => 'crawl', 'after_space_id' => $after, 'tasks_total' => (int)($crawl['tasks_total'] ?? $alreadyCrawledTasks)], JSON_UNESCAPED_UNICODE), $leaseToken);
                $this->repo->updateProgress($jobPublicId, 'crawl_checkpoint', min(95, (float)($crawl['spaces'] ?? 0)), $crawl, $leaseToken);
                ...
                if (!$this->repo->requestStatus($jobPublicId, 'queued')) throw new RuntimeException('CLICKUP_CRAWL_REQUEUE_FAILED');
                return;
            }
        ...
        while (($items = $this->repo->importItemsBatch((int)$job['id'], $priority, $lastId, 100)) !== []) {
            foreach ($items as $item) {
                ...
                // The crawler may generate a stable synthetic source ID when
                // ClickUp omits one (for example comment attachments and
                // checklist items). Keep the job-item ID in the payload so the
                // writer and mapping layer use exactly the same idempotency key.
                $payload['_source_id'] = (string)$item['source_id'];
                if (in_array($type, ['space', 'folder', 'list', 'task', 'comment', 'attachment', 'checklist', 'checklist_item', 'time_entry', 'dependency'], true)) $payload['_source_project_id'] = (string)($item['source_project_id'] ?? $payload['_source_project_id'] ?? '');
                if ($type === 'task') {
                    $payload['_source_parent_id'] = (string)($item['source_parent_id'] ?? $payload['parent'] ?? $payload['parent_id'] ?? '');
                    $mapType = 'task';
                } else {
                    $mapType = $type;
                }
                ...
                try {
                    $result = match ($type) {
                        'space' => $this->writer->project($job, $payload, $actor),
                        'folder', 'list' => $this->writer->module($job, $payload + ['_source_type' => $type], $actor),
                        'tag' => $this->writer->label($job, $payload),
                        'task' => $this->writer->task($job, $payload, $actor),
                        'checklist' => $this->writer->checklist($job, $payload, $actor),
                        'checklist_item' => $this->writer->checklistItem($job, $payload, $actor),
                        ...
                        'goal' => $this->writer->goal($job, $payload, $actor),
                    ...
                    $parentFailure = str_contains($e->getMessage(), 'PARENT_TASK_NOT_READY');
                    $this->repo->upsertItem((int)$job['id'], $type, (string)$item['source_id'], ['status' => 'failed', 'attempts' => (int)($item['attempts'] ?? 0) + 1, 'error_code' => $parentFailure ? 'PARENT_TASK_NOT_READY' : 'IMPORT_FAILED', 'error_message' => 'Item import failed. Check the migration log.']);
                    $this->repo->addLog((int)$job['id'], 'error', 'import_' . $type, 'Source item import failed.', ['source_type' => $type, 'source_id' => $item['source_id'], 'error_code' => $e->getCode() ?: 'CLICKUP_IMPORT_ERROR']);
                }
                ++$done;
        ...
        $parentRetry = max(0, (int)($existing['parent_retry'] ?? 0));
        if ($parentRetry < 100 && $this->repo->resetParentFailures((int)$job['id']) > 0) {
            $this->repo->updateCursor($jobPublicId, json_encode(['phase' => 'import', 'priority' => 0, 'id' => 0, 'parent_retry' => $parentRetry + 1], JSON_UNESCAPED_UNICODE), $leaseToken);
            $this->processJob($jobPublicId, $leaseToken);
            return;
        }
        ...
        $this->repo->addLog((int)$job['id'], 'info', 'completed', 'ClickUp migration completed.', ['failed' => $failed, 'warnings' => count($summary['warnings'])]);
    }

    private function configureOAuthRefresh(array &$connection, string &$token): void
    {
        // ClickUp OAuth access tokens are currently non-expiring. Keep the
        // hook intentionally empty so a future token-rotation contract can be
        // added without pretending that an unsupported refresh flow exists.
    }

    public function rollback(string $jobPublicId, array $actor): void
    {
        $job = $this->repo->beginRollback($jobPublicId);
        if (!$job) throw new RuntimeException('CLICKUP_ROLLBACK_REQUIRES_TERMINAL_JOB');
        ...
        try {
            ...
            while (($items = $this->repo->rollbackItemsBatch((int)$job['id'], $before, 100)) !== []) {
                $batchWarning = false;
                foreach ($items as $item) {
                    if (!$this->repo->heartbeat($jobPublicId, $lease)) throw new RuntimeException('CLICKUP_ROLLBACK_LEASE_LOST');
                    ...
                    if (!$isProjectCommentMutation && $this->repo->targetReferencedByOtherJob((int)$job['id'], (string)$item['target_type'], (string)$item['target_public_id'])) {
                        $this->repo->upsertItem((int)$job['id'], (string)$item['source_type'], (string)$item['source_id'], ['status' => 'rollback_preserved_shared', 'error_code' => 'SHARED_TARGET']);
                        continue;
                    }
                    try {
                        if ($isProjectCommentMutation) {
                            ...
                            if ($expectedDescription === null) {
                                throw new RuntimeException('CLICKUP_ROLLBACK_PROJECT_SNAPSHOT_MISSING');
                            }
                            ...
                            if (!is_array($currentProject) || (string)($currentProject['description'] ?? '') !== $expectedDescription) {
                                throw new RuntimeException('CLICKUP_ROLLBACK_PROJECT_CHANGED');
                            }
                            ...
                            if (!is_array($restored)) throw new RuntimeException('CLICKUP_ROLLBACK_PROJECT_UPDATE_FAILED');
                        } else {
                            $serviceId = match ((string)$item['target_type']) { 'project' => 'service.project', 'task' => 'service.task', 'file' => 'service.file', 'project_module' => 'service.project_module', 'tag' => 'service.tag', 'comment' => 'service.comment', 'checklist', 'checklist_item' => 'service.checklist', 'worklog' => 'service.worklog', 'dependency' => 'service.dependency', default => '' };
                            if ($serviceId === '') continue;
                            $service = $this->writer->service($serviceId);
                            $deleted = match ((string)$item['target_type']) {
                            ...
                            if ($deleted !== true) throw new RuntimeException('CLICKUP_ROLLBACK_TARGET_NOT_DELETED');
                        }
                        ...
                        $this->repo->addLog((int)$job['id'], 'warning', 'rollback', 'Target was not removed.', ['source_id' => $item['source_id'], 'error_code' => $e->getCode() ?: 'CLICKUP_ROLLBACK_TARGET_NOT_DELETED']);
                    }
                }
                // Continue through the complete batch even when one target
                // cannot be removed. Otherwise a single warning would strand
                // all later rollback items behind the same cursor forever.
