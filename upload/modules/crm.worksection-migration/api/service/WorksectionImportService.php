...
final class WorksectionImportService
{
    public function __construct(
        private readonly WorksectionMigrationRepository $repo,
        private readonly WorksectionClient $client,
        private readonly WorksectionCrawler $crawler,
        private readonly WorksectionTargetWriter $writer,
    ) {
    }
    ...
    public function processJob(string $jobPublicId, ?string $leaseToken = null): void
    {
        ...
        if (!$connection) throw new RuntimeException('WORKSECTION_CONNECTION_NOT_FOUND');
        $token = $this->resolveToken($connection);
        $this->client->setAccountUrl((string)($connection['account_url'] ?? ''));
        $this->client->setAuthType((string)($connection['auth_type'] ?? 'api_key'));
        ...
        if (!$resume) {
            $this->repo->updateProgress($jobPublicId, 'crawl', 0, ['message'=>'Загрузка Worksection'], $leaseToken);
            $crawl = $this->crawler->crawl($job, $token, $heartbeat);
            $this->repo->addLog((int)$job['id'], 'info', 'crawl', 'Worksection graph loaded.', $crawl);
        ...
        while(($items=$this->repo->importItemsBatch((int)$job['id'],$priority,$lastId,250))!==[]) {
            foreach($items as $item) {
                if($leaseToken!==null&&!$this->repo->heartbeat($jobPublicId,$leaseToken))throw new RuntimeException('WORKSECTION_JOB_LEASE_LOST');
                ...
                if(in_array($type,['project','task','subtask'],true))$payload['_source_project_id']=(string)($item['source_project_id']??$payload['project_id']??'');
                ...
                $mappingType=match($type){'subtask'=>'task','project_group'=>'project_group','label'=>'label',default=>$type};
                ...
                try {
                    ...
                    $result=match($type){
                        'project_group'=>$this->writer->projectGroup($job,$payload,$actor),
                    ...
                    $this->repo->addLog((int)$job['id'],'error','import_'.$type,'Worksection item import failed.',['source_type'=>$type,'source_id'=>$item['source_id'],'error_code'=>$e->getCode()?:$e->getMessage()]);
                }
    ...
    private function resolveToken(array $connection): string
    {
        $authType = (string)($connection['auth_type'] ?? 'api_key');
        $token = EncryptionService::decrypt((string)($connection['api_key_encrypted'] ?? ''));
        if ($authType === 'oauth2') $token = EncryptionService::decrypt((string)($connection['access_token_encrypted'] ?? ''));
        if ($token === null || $token === '') throw new RuntimeException('WORKSECTION_CREDENTIAL_DECRYPT_FAILED');
        return $token;
    }
}
