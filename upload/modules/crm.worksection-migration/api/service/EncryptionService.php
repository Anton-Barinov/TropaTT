...
final class EncryptionService
{
    private static function key(): string
    {
        ...
        if ($secret === '') {
            throw new RuntimeException('APP_SECRET is not configured for Worksection credential encryption');
        }
        return hash_hkdf('sha256', $secret, 32, 'worksection-migration');
    }
    ...
    public static function encrypt(string $plaintext): string
    {
        ...
        if (!is_string($ciphertext) || $ciphertext === '') {
            throw new RuntimeException('Failed to encrypt Worksection credential');
        }
