...
namespace Module\Crm\WorksectionMigration\Repository;

use PDO;
use RuntimeException;

final class WorksectionMigrationRepository
{
    ...
    {
        $sql = 'SELECT id,public_id,name,auth_type,account_url,workspace_gid,status,last_checked_at,last_error,created_by_user_id,created_at,updated_at FROM module_worksection_connections';
        $params = [];
    ...
    {
        $stmt = $this->pdo->prepare('SELECT * FROM module_worksection_connections WHERE public_id=:id LIMIT 1');
    ...
    {
        $stmt = $this->pdo->prepare('SELECT * FROM module_worksection_connections WHERE id=:id LIMIT 1');
        $stmt->execute(['id' => $id]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC); return is_array($row) ? $row : null;
    }
    ...
    {
        $publicId = $this->id('wsc'); $now = $this->now();
        $stmt = $this->pdo->prepare('INSERT INTO module_worksection_connections (public_id,name,auth_type,account_url,api_key_encrypted,access_token_encrypted,refresh_token_encrypted,client_id_encrypted,client_secret_encrypted,status,created_by_user_id,created_at,updated_at) VALUES (:public_id,:name,:auth,:account_url,:api_key,:access,:refresh,:client,:secret,\'draft\',:owner,:created,:updated)');
        $stmt->execute(['public_id' => $publicId, 'name' => $data['name'], 'auth' => $data['auth_type'] ?? 'api_key', 'account_url' => $data['account_url'] ?? '', 'api_key' => $data['api_key_encrypted'] ?? null, 'access' => $data['access_token_encrypted'] ?? null, 'refresh' => $data['refresh_token_encrypted'] ?? null, 'client' => $data['client_id_encrypted'] ?? null, 'secret' => $data['client_secret_encrypted'] ?? null, 'owner' => $data['created_by_user_id'], 'created' => $now, 'updated' => $now]);
        return $this->getConnection($publicId) ?? ['public_id' => $publicId];
    }

    public function updateConnection(string $publicId, array $data): void
    {
        $allowed = ['name','account_url','api_key_encrypted','access_token_encrypted','refresh_token_encrypted','client_id_encrypted','client_secret_encrypted','workspace_gid'];
        ...
        $this->pdo->prepare('UPDATE module_worksection_connections SET ' . implode(',', $sets) . ' WHERE public_id=:id')->execute($params);
    }
    ...
    public function deleteConnection(int $connectionId): void
    {
        ...
        try {
            foreach (['module_worksection_unresolved_entities' => 'job_id IN (SELECT id FROM module_worksection_jobs WHERE connection_id=:id)', 'module_worksection_job_logs' => 'job_id IN (SELECT id FROM module_worksection_jobs WHERE connection_id=:id)', 'module_worksection_job_items' => 'job_id IN (SELECT id FROM module_worksection_jobs WHERE connection_id=:id)', 'module_worksection_jobs' => 'connection_id=:id', 'module_worksection_source_mappings' => 'connection_id=:id', 'module_worksection_user_mappings' => 'connection_id=:id', 'module_worksection_rate_limits' => 'connection_id=:id'] as $table => $where) $this->pdo->prepare('DELETE FROM ' . $table . ' WHERE ' . $where)->execute(['id' => $connectionId]);
            $this->pdo->prepare('DELETE FROM module_worksection_connections WHERE id=:id')->execute(['id' => $connectionId]);
            $this->pdo->commit();
    ...
    {
        $stmt = $this->pdo->prepare("SELECT id FROM module_worksection_jobs WHERE connection_id=:id AND status IN ('queued','running','pausing','cancelling','rolling_back') LIMIT 1"); $stmt->execute(['id' => $connectionId]); return $stmt->fetchColumn() !== false;
    }

    public function rateState(int $connectionId): ?array
    {
        $stmt = $this->pdo->prepare('SELECT * FROM module_worksection_rate_limits WHERE connection_id=:id LIMIT 1'); $stmt->execute(['id' => $connectionId]); $row = $stmt->fetch(PDO::FETCH_ASSOC); return is_array($row) ? $row : null;
    }
    ...
    {
        $stmt = $this->pdo->prepare('INSERT INTO module_worksection_rate_limits (connection_id,requests_made,window_started_at,retry_after_until,last_http_status,last_request_at,updated_at) VALUES (:id,:requests,:window,:retry,:status,:last_request,:updated) ON DUPLICATE KEY UPDATE requests_made=VALUES(requests_made),window_started_at=VALUES(window_started_at),retry_after_until=VALUES(retry_after_until),last_http_status=VALUES(last_http_status),last_request_at=VALUES(last_request_at),updated_at=VALUES(updated_at)');
    ...
    {
        $publicId = $this->id('wsj'); $now = $this->now();
        $stmt = $this->pdo->prepare('INSERT INTO module_worksection_jobs (public_id,connection_id,workspace_gid,mode,status,source_scope_json,target_options_json,created_by_user_id,created_at,updated_at) VALUES (:public_id,:connection,:workspace,:mode,\'draft\',:scope,:options,:owner,:created,:updated)');
    ...
    {
        $stmt = $this->pdo->prepare('SELECT j.*,c.name AS connection_name FROM module_worksection_jobs j JOIN module_worksection_connections c ON c.id=j.connection_id WHERE j.public_id=:id LIMIT 1'); $stmt->execute(['id' => $publicId]); $row = $stmt->fetch(PDO::FETCH_ASSOC); return is_array($row) ? $this->decodeJob($row) : null;
    }

    public function listJobs(int $actorId, bool $manager): array
    {
        $sql = 'SELECT j.*,c.name AS connection_name FROM module_worksection_jobs j JOIN module_worksection_connections c ON c.id=j.connection_id'; $params = [];
    ...
    public function updateJobStatus(string $publicId, string $status, ?string $leaseToken = null): void
    {
        ...
        $stmt = $this->pdo->prepare('UPDATE module_worksection_jobs SET ' . implode(',', $sets) . $where); $stmt->execute($params);
        if ($leaseToken !== null && !$this->leaseTokenMatches($publicId, $leaseToken)) throw new RuntimeException('WORKSECTION_JOB_LEASE_LOST');
    }

    public function requestStatus(string $publicId, string $status): bool
    {
        $stmt = $this->pdo->prepare("UPDATE module_worksection_jobs SET status=:status,updated_at=UTC_TIMESTAMP() WHERE public_id=:id AND status IN ('draft','queued','running','paused','pausing','cancelling','failed','cancelled','completed','completed_with_warnings')");
    ...
    public function beginRollback(string $publicId, int $leaseSeconds = 900): ?array
    {
        $this->pdo->beginTransaction();
        try {
            $stmt = $this->pdo->prepare('SELECT id,status,lease_until,last_source_cursor FROM module_worksection_jobs WHERE public_id=:id LIMIT 1 FOR UPDATE');
            $stmt->execute(['id' => $publicId]);
            ...
            $update=$this->pdo->prepare("UPDATE module_worksection_jobs SET status='rolling_back',lease_token=:token,lease_until=:until,last_source_cursor=:cursor,updated_at=UTC_TIMESTAMP() WHERE id=:id AND status=:expected");
    ...
    public function updateProgress(string $publicId, string $step, float $percent, array $progress, ?string $leaseToken = null): void
    {
        ...
        if ($leaseToken !== null) { $where .= ' AND lease_token=:lease AND lease_until >= UTC_TIMESTAMP() AND status IN (\'running\',\'rolling_back\')'; $params['lease'] = $leaseToken; }
        $stmt = $this->pdo->prepare('UPDATE module_worksection_jobs SET current_step=:step,progress_percent=:percent,progress_json=:progress,updated_at=:updated' . $where); $stmt->execute($params);
        if ($leaseToken !== null && !$this->ownsLease($publicId, $leaseToken)) throw new RuntimeException('WORKSECTION_JOB_LEASE_LOST');
    }
    ...
    public function updateSummary(string $publicId, array $summary, ?string $leaseToken = null): void
    {
        ...
        if ($leaseToken !== null) { $where .= ' AND lease_token=:lease AND lease_until >= UTC_TIMESTAMP() AND status IN (\'running\',\'rolling_back\')'; $params['lease'] = $leaseToken; }
        $stmt = $this->pdo->prepare('UPDATE module_worksection_jobs SET summary_json=:summary,updated_at=:updated' . $where); $stmt->execute($params);
        if ($leaseToken !== null && !$this->ownsLease($publicId, $leaseToken)) throw new RuntimeException('WORKSECTION_JOB_LEASE_LOST');
    }
    ...
    public function claimNextJob(int $leaseSeconds = 240): ?array
    {
        $this->pdo->beginTransaction();
        try {
            $stmt = $this->pdo->query("SELECT j.id FROM module_worksection_jobs j WHERE (j.status='queued' OR (j.status='running' AND j.lease_until IS NOT NULL AND j.lease_until < UTC_TIMESTAMP())) AND NOT EXISTS (SELECT 1 FROM module_worksection_jobs active WHERE active.connection_id=j.connection_id AND (active.status='rolling_back' OR (active.status='running' AND active.lease_until IS NOT NULL AND active.lease_until >= UTC_TIMESTAMP()))) ORDER BY j.created_at ASC LIMIT 1 FOR UPDATE");
            $id = $stmt->fetchColumn();
            if ($id === false) { $this->pdo->commit(); return null; }
            $token = bin2hex(random_bytes(16)); $until = gmdate('Y-m-d H:i:s', time() + $leaseSeconds);
            $this->pdo->prepare("UPDATE module_worksection_jobs SET status='running',lease_token=:token,lease_until=:until,started_at=COALESCE(started_at,UTC_TIMESTAMP()),updated_at=UTC_TIMESTAMP() WHERE id=:id")->execute(['token' => $token, 'until' => $until, 'id' => $id]);
            $this->pdo->commit();
            $stmt = $this->pdo->prepare('SELECT j.*,c.name AS connection_name FROM module_worksection_jobs j JOIN module_worksection_connections c ON c.id=j.connection_id WHERE j.id=:id'); $stmt->execute(['id' => $id]); $row = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    {
        $stmt = $this->pdo->prepare("UPDATE module_worksection_jobs SET lease_until=:until,updated_at=UTC_TIMESTAMP() WHERE public_id=:id AND lease_token=:token AND lease_until >= UTC_TIMESTAMP() AND status IN ('running','rolling_back')"); $stmt->execute(['until' => gmdate('Y-m-d H:i:s', time() + $leaseSeconds), 'id' => $publicId, 'token' => $leaseToken]); return $this->ownsLease($publicId, $leaseToken);
    }

    public function ownsLease(string $publicId, string $leaseToken): bool
    {
        $stmt = $this->pdo->prepare("SELECT id FROM module_worksection_jobs WHERE public_id=:id AND lease_token=:token AND status IN ('running','rolling_back') AND lease_until >= UTC_TIMESTAMP() LIMIT 1"); $stmt->execute(['id' => $publicId, 'token' => $leaseToken]); return $stmt->fetchColumn() !== false;
    }

    private function leaseTokenMatches(string $publicId, string $leaseToken): bool
    {
        $stmt = $this->pdo->prepare('SELECT id FROM module_worksection_jobs WHERE public_id=:id AND lease_token=:token AND lease_until >= UTC_TIMESTAMP() LIMIT 1');
        $stmt->execute(['id' => $publicId, 'token' => $leaseToken]);
        return $stmt->fetchColumn() !== false;
    }

    public function releaseLease(string $publicId, string $leaseToken): void
    { $this->pdo->prepare('UPDATE module_worksection_jobs SET lease_token=NULL,lease_until=NULL,updated_at=UTC_TIMESTAMP() WHERE public_id=:id AND lease_token=:token')->execute(['id' => $publicId, 'token' => $leaseToken]); }
    ...
    public function retryJob(string $publicId): ?int
    {
        $this->pdo->beginTransaction();
        try {
            $jobStmt=$this->pdo->prepare("SELECT id FROM module_worksection_jobs WHERE public_id=:id AND status IN ('completed_with_warnings','failed','cancelled') LIMIT 1 FOR UPDATE");
            $jobStmt->execute(['id'=>$publicId]);$job=$jobStmt->fetch(PDO::FETCH_ASSOC);
            if(!is_array($job)){$this->pdo->commit();return null;}
            $items=$this->pdo->prepare("UPDATE module_worksection_job_items SET status='pending',error_code=NULL,error_message=NULL,updated_at=UTC_TIMESTAMP() WHERE job_id=:job AND status='failed'");$items->execute(['job'=>(int)$job['id']]);$count=$items->rowCount();
            $this->pdo->prepare("UPDATE module_worksection_jobs SET status='queued',last_source_cursor=NULL,updated_at=UTC_TIMESTAMP() WHERE id=:id")->execute(['id'=>(int)$job['id']]);
    ...
    { $stmt = $this->pdo->prepare('SELECT * FROM module_worksection_job_items WHERE job_id=:job AND source_type=:type AND source_id=:source LIMIT 1'); $stmt->execute(['job' => $jobId, 'type' => $type, 'source' => $sourceId]); $row = $stmt->fetch(PDO::FETCH_ASSOC); return is_array($row) ? $row : null; }

    public function items(int $jobId, ?string $status = null, int $limit = 5000): array
    { $sql = 'SELECT * FROM module_worksection_job_items WHERE job_id=:job'; $params = ['job' => $jobId]; if ($status !== null && $status !== '') { $sql .= ' AND status=:status'; $params['status'] = $status; } $sql .= ' ORDER BY id ASC LIMIT ' . max(1, min(10000, $limit)); $stmt = $this->pdo->prepare($sql); $stmt->execute($params); return $stmt->fetchAll(PDO::FETCH_ASSOC) ?: []; }

    /** @return array<int,array<string,mixed>> */
    public function importItemsBatch(int $jobId, int $priority = 0, int $lastId = 0, int $limit = 250): array
    {
        $limit = max(1, min(1000, $limit));
        $sql = "SELECT * FROM (SELECT i.*, CASE i.source_type WHEN 'project' THEN 10 WHEN 'label' THEN 20 WHEN 'task' THEN 30 WHEN 'subtask' THEN 35 WHEN 'comment' THEN 60 WHEN 'attachment' THEN 70 WHEN 'time_record' THEN 80 ELSE 90 END AS import_priority FROM module_worksection_job_items i WHERE i.job_id=:job AND i.status='pending') AS ordered_items WHERE (ordered_items.import_priority>:priority OR (ordered_items.import_priority=:same_priority AND ordered_items.id>:last_id)) ORDER BY ordered_items.import_priority ASC, ordered_items.id ASC LIMIT {$limit}";
    ...
    {
        $stmt = $this->pdo->prepare('SELECT * FROM module_worksection_job_items WHERE job_id=:job AND created_by_job=1 AND target_public_id IS NOT NULL AND status NOT IN (\'rolled_back\',\'rollback_preserved_shared\') AND id<:before_id ORDER BY id DESC LIMIT ' . max(1, min(1000, $limit)));
    ...
    {
        $stmt = $this->pdo->prepare("SELECT i.id FROM module_worksection_job_items i INNER JOIN module_worksection_jobs j ON j.id=i.job_id WHERE i.job_id<>:job AND i.target_type=:type AND i.target_public_id=:target AND i.status NOT IN ('rolled_back','rollback_preserved_shared') LIMIT 1");
    ...
    public function updateCursor(string $publicId, string $cursor, ?string $leaseToken = null): void
    {
        ...
        $where = ' WHERE public_id=:id';
        if ($leaseToken !== null) { $where .= ' AND lease_token=:lease AND lease_until >= UTC_TIMESTAMP() AND status IN (\'running\',\'rolling_back\')'; $params['lease'] = $leaseToken; }
        $stmt = $this->pdo->prepare('UPDATE module_worksection_jobs SET last_source_cursor=:cursor,updated_at=UTC_TIMESTAMP()' . $where);
        $stmt->execute($params);
        if ($leaseToken !== null && !$this->ownsLease($publicId, $leaseToken)) throw new RuntimeException('WORKSECTION_JOB_LEASE_LOST');
    }

    public function itemCounts(int $jobId): array
    { $stmt = $this->pdo->prepare('SELECT status,COUNT(*) AS count FROM module_worksection_job_items WHERE job_id=:job GROUP BY status'); $stmt->execute(['job' => $jobId]); $out = []; foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) ?: [] as $row) $out[(string)$row['status']]=(int)$row['count']; return $out; }

    public function itemCount(int $jobId): int
    { $stmt = $this->pdo->prepare('SELECT COUNT(*) FROM module_worksection_job_items WHERE job_id=:job'); $stmt->execute(['job' => $jobId]); return (int)$stmt->fetchColumn(); }

    public function findMapping(int $connectionId, string $workspace, string $type, string $sourceId): ?array
    { $stmt = $this->pdo->prepare('SELECT * FROM module_worksection_source_mappings WHERE connection_id=:connection AND workspace_gid=:workspace AND source_type=:type AND source_id=:source LIMIT 1'); $stmt->execute(['connection' => $connectionId, 'workspace' => $workspace, 'type' => $type, 'source' => $sourceId]); $row = $stmt->fetch(PDO::FETCH_ASSOC); return is_array($row) ? $row : null; }
    ...
    { $stmt = $this->pdo->prepare("SELECT public_id FROM tasks WHERE source_type='worksection' AND source_id=:source AND deleted_at IS NULL ORDER BY id DESC LIMIT 1"); $stmt->execute(['source'=>$sourceId]); $value=$stmt->fetchColumn(); return $value===false?null:(string)$value; }
    ...
    public function mappedUserId(int $connectionId,string $worksectionGid): ?int
    { $stmt=$this->pdo->prepare('SELECT u.id FROM module_worksection_user_mappings m JOIN users u ON u.public_id=m.crm_user_public_id WHERE m.connection_id=:connection AND m.worksection_user_gid=:gid AND m.mapping_status=\'mapped\' AND u.is_active=1 LIMIT 1'); $stmt->execute(['connection'=>$connectionId,'gid'=>$worksectionGid]); $value=$stmt->fetchColumn(); return $value===false?null:(int)$value; }

    public function mappedUserPublicId(int $connectionId,string $worksectionGid): ?string
    { $stmt=$this->pdo->prepare('SELECT u.public_id FROM module_worksection_user_mappings m JOIN users u ON u.public_id=m.crm_user_public_id WHERE m.connection_id=:connection AND m.worksection_user_gid=:gid AND m.mapping_status=\'mapped\' AND u.is_active=1 LIMIT 1'); $stmt->execute(['connection'=>$connectionId,'gid'=>$worksectionGid]); $value=$stmt->fetchColumn(); return $value===false?null:(string)$value; }

    public function addLog(int $jobId,string $level,string $step,string $message,array $context=[]): void
    { $stmt=$this->pdo->prepare('INSERT INTO module_worksection_job_logs (job_id,level,step,message,context_json,created_at) VALUES (:job,:level,:step,:message,:context,:created)'); $stmt->execute(['job'=>$jobId,'level'=>$level,'step'=>$step,'message'=>mb_substr($message,0,2000),'context'=>$context===[]?null:$this->json($context),'created'=>$this->now()]); }
    public function logs(int $jobId,int $limit=100): array
    { $stmt=$this->pdo->prepare('SELECT * FROM module_worksection_job_logs WHERE job_id=:job ORDER BY id DESC LIMIT '.max(1,min(1000,$limit))); $stmt->execute(['job'=>$jobId]); return $stmt->fetchAll(PDO::FETCH_ASSOC)?:[]; }
