...
| PATCH | `/api/v1/chats/{public_id}/messages/{message_public_id}` | Edit message (author, within 60 min) | Yes | `chat.use` | History recorded; external users (client portal) may edit only their own message in a `project_client` chat they participate in |
| DELETE | `/api/v1/chats/{public_id}/messages/{message_public_id}` | Delete message (author, within 10 min) | Yes | `chat.use` | Soft-delete; external users (client portal) may delete only their own message in a `project_client` chat they participate in |
