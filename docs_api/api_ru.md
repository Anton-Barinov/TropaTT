...
> Последнее обновление: 2026-08-20. Источник данных: `upload/api/config/routes.php` и файлы маршрутов модулей.
...
| Метод | Endpoint | Назначение | Auth | Permissions | Описание |
|-------|----------|------------|:---:|-------------|----------|
| POST | `/api/v1/external-users/invite` | Пригласить контакт в клиентский портал | Да | `contact.manage` | Создаёт или повторно приглашает аккаунт `external_guest`; требуется доступ к контакту, привязанный контрагент и валидный email. Ссылка действует 7 дней |
| POST | `/api/v1/external-users/accept` | Принять приглашение, задать пароль | Нет | — | Публичный; требует `token` приглашения + `password` (8–1024 символа). Токен одноразовый; ответ выдаёт временную сессию и данные логина |
| GET | `/api/v1/external-users` | Список внешних (портальных) пользователей | Да | `contact.manage` | Root видит всех; остальные — только внешних пользователей доступных им контактов |
| POST | `/api/v1/external-users/{public_id}/deactivate` | Отозвать доступ в портал | Да | `contact.manage` | Деактивирует аккаунт, обнуляет приглашение и отзывает все его сессии |
