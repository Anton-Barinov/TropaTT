...
- **Быстрые фильтры активности**:
  ...
  - `include_ancestors=1`: В связке с фильтром `status` дополнительно возвращает родительские задачи дочерних, попавших в выборку, даже если родитель находится в другом статусе. Предназначен для иерархического/дерево вида, чтобы структура вложенности сохранялась при фильтрации по статусу. Учитываются только **видимые** дочерние задачи — мягко удалённая, архивная подзадача или подзадача архивного проекта не вытягивает родителя в список, — а `exclude_statuses` / `hide_done` ослабляется только для строк-родителей: завершённая задача возвращается исключительно как рамка дерева для подходящей дочерней, но не отдельной строкой.
  - `hierarchy=1`: Включает пагинацию целыми корневыми группами. `limit` задаёт число корневых групп на странице, а ответ содержит все доступные вложенные задачи этих групп. При активных фильтрах API добавляет доступные цепочки предков; сведения о группах, совпадениях и фактически возвращённых задачах находятся в `meta.hierarchy`. Курсорная пагинация в этом режиме заменяется offset-пагинацией.
...
| Метод | Endpoint | Назначение | Auth | Permissions | Описание |
|-------|----------|------------|:---:|-------------|----------|
| POST | `/api/v1/external-users/invite` | Пригласить контакт в клиентский портал | Да | `contact.manage` | Создаёт аккаунт `external_guest` для контакта; требуется привязанный контрагент и валидный email. Тело: `external_role` (`observer` \| `executor`, по умолчанию `observer`) |
...
| GET | `/api/v1/external-users` | Список внешних (портальных) пользователей | Да | `contact.manage` | Каждый элемент включает `external_role` |
...
| GET | `/api/v1/external-users/{public_id}/project-access` | Список проектов, выданных исполнителю | Да | `contact.manage` | Только для роли executor |
| POST | `/api/v1/external-users/{public_id}/project-access` | Выдать доступ к проекту исполнителю | Да | `project.manage` | Тело: `project_public_id`. Идемпотентно (уникально по паре пользователь+проект) |
| DELETE | `/api/v1/external-users/{public_id}/project-access/{project_public_id}` | Отозвать доступ к проекту | Да | `project.manage` | — |

**Модель доступа:** внешний пользователь (`is_external = true`) входит под своим приглашённым email в качестве логина и имеет одну из двух ролей (`external_role`):

- **`observer`** (по умолчанию) — через row-level security видит только проекты и задачи своего контрагента (`ProjectService`/`TaskService` фильтруют по `client_public_id` на уровне SQL, а не в PHP-массиве). Это классический режим клиентского портала для просмотра и комментирования.
- **`executor`** — фрилансер/подрядчик, доступ которого вместо этого определяется явными выдачами по каждому проекту в таблице `external_user_project_access` (см. endpoint-ы project-access выше). Выдачи узкие (по одному проекту), аудируемые и отзываемые; они не привязаны к конкретному контрагенту, поэтому один аккаунт-исполнитель может получить доступ к проектам разных, не связанных между собой контрагентов, ни разу не получив видимость всего контрагента целиком. При приглашении исполнителю автоматически выдаются все неархивные проекты приглашающего контрагента; остальные проекты (в том числе от других контрагентов) выдаются явно через `POST .../project-access`.

Помимо проверки прав, жёсткий allowlist маршрутов (`external_ok` в `routes.php`, применяется централизованно в `App::run()`) ограничивает внешние сессии до `auth/me`, `auth/logout`, `auth/menu`, `projects` (list/get), `tasks` (list/get/create/comments/files), `files` (upload/get/download) и `notifications` — любой другой endpoint вернёт `403 EXTERNAL_ACCESS_DENIED`, даже если проверка прав сама по себе разрешила бы доступ. Второй, более узкий allowlist (`external_executor_ok`, на `GET`/`POST /api/v1/worklogs`) дополнительно требует `external_role === 'executor'`, поэтому логировать время по задаче может только исполнитель — наблюдатель никогда, даже несмотря на общий `is_external = true`. Веб-приложение зеркалирует это на уровне страниц: меню и доступные маршруты для внешних аккаунтов ограничены разделами «Проекты», «Задачи», «Уведомления».
