...
- Individual groups: `sticky`, `modules`, `knowledge`, `cycles`, `companies`, `updates` (`UpdaterStepMachineMysqlIntegrationTest` — end-to-end apply/rollback of the updater step machine against a live local MySQL: chunked dump/restore, multi-request migrations, DB restore on rollback), `trello` (`TrelloMigrationIntegrationTest` — opt-in live Trello authentication/discovery plus module persistence and rollback orchestration; requires an explicit dedicated test database and `TRELLO_TEST_CONFIRM=1`), `asana` (`AsanaMigrationIntegrationTest` — opt-in Asana PAT authentication, workspace discovery and temporary connection cleanup; requires an explicit dedicated test database and `ASANA_TEST_CONFIRM=1`).
...
### Commit authorship and attribution (strict rule)

Every commit must be authored and committed **only by the repository owner** (`Anton Barinov <55404783+Anton-Barinov@users.noreply.github.com>`). This is a hard requirement — the GitHub contributor graph and commit history must never show anyone else.

- Never change `git config user.name` / `user.email` to a different identity.
- Never add any "Co-Authored-By", "Generated by", "Author", tool name, or assistant/agent name to a commit message or commit body. No coding tool or AI assistant may ever appear in commit metadata.
- Never create commits that would attribute work to an agent, bot, or third party.
- Never add any person other than the owner to CONTRIBUTORS files, AUTHORS files, or `.github/` contributor metadata.
- If a commit is made under any other identity or carries such an attribution, rewrite it to the owner identity/message immediately.
- Do not create or use a personal access token or `gh` account that would attribute pushes to anyone but the owner.

All code changes, regardless of who writes them, are published to GitHub only through the owner's identity and only under the owner's account.
