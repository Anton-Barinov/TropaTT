...
Стандартная установка поставляется с пустым каталогом модулей — на месте только `.htaccess`.
Модули ставятся по необходимости из официального маркетплейса (`marketplace.tropatt.com`) либо
копированием пакета в `modules/`; готовые примеры опубликованы в `docs_modules/examples/modules/`.
