...
- **The whole request path logs to the application log.** Every remaining `error_log()` in the API controllers, services, models and module subsystem (189 calls in 74 files) and in the web render path (12 calls, with the logger now published by the web bootstrap) goes through `AppLog`, so a failed request leaves a readable record in `storage_api/logs/error.log` instead of a message that PHP-FPM discards. Migrations and seed scripts still use `error_log()`, which is visible in their CLI output where they run.
