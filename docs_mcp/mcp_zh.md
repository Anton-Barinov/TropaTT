...
目录大小会影响每个请求：将会话中注册 MCP 工具的代理客户端会在每个请求中发送工具模式。在线上端点实测：
...
- **全局模式**（启动时注册一次工具）：每个请求都会携带模式。使用默认 `core` 后，每次请求约 4.9K tokens，而不是约 52K——即使不做懒加载也可接受。在服务器 URL 上追加 `?toolset=<域>` 可进一步降低。
- **预设 + lazy 模式**（通过 `mcp_load`/`mcp_call` 按需加载工具）：模式作为普通工具调用结果返回，属于 append-only，不会使请求缓存前缀失效；工具列表本身保持不变（少数控制工具）。配合服务端工具集，`mcp_load` 的结果也很紧凑。适用于 CRM 使用频率低但目标明确的场景。
- **预设 + dynamic 模式**（服务器加载时注册工具）：每次加载都会改变工具定义块，破坏缓存前缀并强制重新评估整个提示词。服务端工具集可同时缩小注册集和加载的模式，但在短会话中 lazy 模式仍然严格更优。
