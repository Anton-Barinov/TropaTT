...
| MCP tools | 620 в полном каталоге (`toolset=all`); `tools/list` по умолчанию возвращает core-профиль (27 инструментов: 8 mega-tools + 3 оркестратора AgentOS + 16 regular) |
| MCP resources | 6 |
| MCP prompts | 0 |

---

## Mega-tools и оркестраторы AgentOS (консолидированные инструменты по намерению)

Core-профиль использует **mega-tools и оркестраторы AgentOS** — консолидированные инструменты, покрывающие целый домен или сквозной сценарий через параметр `action`, заменяя сотни CRUD-инструментов и экономя токены контекста.

| Mega-tool / Оркестратор | Домен | Действия / Назначение |
|-------------------------|-------|-----------------------|
| `crm_task` | Задачи, подзадачи, комментарии, теги, чек-листы, зависимости, связи, доска, активность | list, get, get_by_key, create, update, delete, move, bulk_update, board, activity, + 20 поддействий |
| `crm_project` | Проекты, вехи, циклы, модули, шаблоны, клиентский кабинет | list, get, create, update, delete, summary, risks, timeline, workload, + 25 поддействий |
| `crm_people` | Пользователи, команды, департаменты, роли, приглашения, impersonation | list_users, get_user, create_user, + 20 поддействий |
| `crm_crm` | Клиенты, контрагенты, компании, контакты, организации | list_clients, get_client, create_client, + 20 поддействий |
| `crm_time` | Worklog, календарь, бизнес-календари, праздники, рабочее время | list_worklogs, create_worklog, get_calendar_agenda, + 25 поддействий |
...
Существующие CRUD-тулы (например `crm_list_tasks`, `crm_create_task`) доступны через профиль `all` и вызываются по имени — обратная совместимость сохранена.
...
- Отсутствие заголовка `Authorization` и неверный ключ различаются: в 401-ответе поле `data.auth_status` равно `missing`, если креды не отправлены, и `invalid`, если отправленный токен/ключ не принят
- Заголовок `MCP-Protocol-Version` необязателен; при его отсутствии подразумевается `2025-06-18`
...
Права tool-ов никогда не шире соответствующего REST-маршрута. Инструменты idea-workflow и knowledge-AI вызываются напрямую на контроллере, поэтому каждый объявляет permission своего маршрута: AI debug требует `ai.admin`, чтение — соответствующий `*.view`, запись — соответствующий `*.manage`. Действия внутри mega-тулов проверяются так же: `crm_knowledge`, `crm_people`, `crm_crm`, `crm_ai` и `crm_admin` на уровне тула принимают любое из нескольких прав, поэтому каждое действие несёт своё требование (`crm_knowledge/reindex` → `settings.manage`, `crm_time/list_holidays` → `settings.manage`, `crm_task/list_approvals` → `approval.manage`, `crm_project/list_cabinet_projects` → `client.manage`). Действие без объявленного права отклоняется. Подписки на вебхуки (действия `crm_admin`) получают ключевые события (`task.created`, `project.updated`, `comment.added`, …) автоматически, с подписью HMAC-SHA256 и без секретов/финансовых полей в payload.
...
Полный каталог MCP велик (620 инструментов). Чтобы не загружать его целиком в каждую сессию агента, `tools/list` по умолчанию возвращает курируемый профиль **`core`** (27 инструментов: профиль, поиск, дашборд, уведомления, активность, 8 mega-tools, плюс 3 оркестратора AgentOS). Доступны профили по доменам:

| Профиль | Охват |
|---------|-------|
| `core` (по умолчанию) | Профиль, поиск, дашборд, уведомления, активность, 8 mega-tools + оркестраторы AgentOS (`crm_agent_bundle`, `crm_agent_memory`, `crm_chat`) |
| `tasks` | Задачи, подзадачи, комментарии, теги, чек-листы, зависимости, связи, оценки, доска, saved views, повторяющиеся правила, напоминания, SLA, workflow-правила, согласования |
| `projects` | Проекты, вехи, циклы, модули проекта, Gantt, сводки/риски, шаблоны, клиентский кабинет |
| `kb` | База знаний: пространства, страницы, версии, комментарии, теги, ссылки, файлы, экспорт/импорт, knowledge AI |
| `people` | Пользователи, команды, департаменты, роли, клиенты, контрагенты, компании, контакты, организации, приглашения |
| `time` | Worklog, события календаря, бизнес-календари, праздники, рабочее время |
| `admin` | Настройки, кеш, модули, обновления ядра, ops, API-клиенты, вебхуки, логи, custom fields, intake, импорт/экспорт, корзина, AI-конфигурация и задачи |
| `all` | Полный каталог по правам пользователя (opt-in) |

Запросить профиль можно, добавив `?toolset=tasks` к URL MCP-эндпоинта или передав `"params": {"toolset": "tasks"}` в `tools/list`; значения через запятую объединяются (`?toolset=tasks,projects`). Вызовите `tools/listToolsets` (или прочитайте `tropatt://server/toolsets`) для машиночитаемого каталога с количеством по профилям. Инструменты, не отнесённые ни к одному профилю, остаются вызываемыми по имени и перечислены в `all`.

## Настройка клиента и бюджет токенов

Размер каталога влияет на каждый запрос: клиенты-агенты, регистрирующие MCP-инструменты в сессии, отправляют схемы инструментов с каждым запросом. Замеры на живом эндпоинте:

| Toolset | Инструментов | Размер схем |
|---------|-------------:|------------:|
| `core` (по умолчанию) | 27 | ~2,9K токенов |
| `tasks` | 82 | ~8,0K токенов |
| `projects` | 58 | ~5,2K токенов |
| `kb` | 80 | ~6,3K токенов |
| `people` | 60 | ~5,7K токенов |
| `time` | 32 | ~2,6K токенов |
| `admin` | 149 | ~12,2K токенов |
| `all` | 620 | ~53K токенов |

Полный каталог примерно в 18 раз дороже профиля `core` по умолчанию, поэтому для сессий, сфокусированных на одном домене, лучше узкий профиль.

Режимы загрузки на клиенте и их влияние на стоимость:

- **Глобальный режим** (инструменты регистрируются один раз при старте): схемы уходят в каждый запрос. С профилем `core` по умолчанию это ~4,9K токенов на запрос вместо ~52K — терпимо даже без ленивой загрузки. Добавьте `?toolset=<домен>` в URL сервера, чтобы сократить ещё сильнее.
- **Пресет + lazy** (инструменты загружаются по требованию через `mcp_load`/`mcp_call`): схемы приходят как обычный результат вызова — это append-only и не инвалидирует префикс кеша запросов; сам набор инструментов постоянен (несколько контрольных инструментов). С серверными toolsets результат `mcp_load` тоже компактный. Рекомендуется, когда CRM используется редко, но точечно.
- **Пресет + dynamic** (инструменты регистрируются при загрузке сервера): блок определений инструментов меняется при каждой загрузке, ломая префикс кеша и заставляя полностью переоценивать промпт. Серверные toolsets уменьшают и регистрируемый набор, и загружаемые схемы, но lazy остаётся строго лучше для коротких сессий.

Серверная фильтрация по toolsets помогает во всех трёх режимах; самый дешёвый вариант — узкий `?toolset=` в URL эндпоинта, при желании в сочетании с ленивой загрузкой.

## Обнаружение документации

Новая сессия агента не знает, где лежит справка по API/MCP, и иначе начинает перечитывать исходный код, чтобы восстановить контракт. Сервер сообщает о документации в двух местах:

- **`initialize.instructions`** — каждый ответ MCP `initialize` содержит строку `instructions` со ссылками на документацию и рекомендуемым порядком работы. Клиенты, поддерживающие спецификацию (Claude Code и другие), показывают её модели автоматически.
- **Ресурс `tropatt://server/docs`** — машиночитаемый каталог тех же ссылок (REST API, MCP, Modules SDK; HTML + raw) через `resources/read`.

Публичный REST-эндпоинт `GET /api/v1/version` также возвращает объект `documentation` с этими ссылками, так что клиент может узнать о доках ещё до получения токена. На форках и приватных установках базовый URL переопределяется переменной окружения `TROPATT_DOCS_URL`.
...
### Tool-уровневые ошибки

Неверные входные данные и неизвестные `action` у mega-tools возвращаются **внутри результата вызова** (`result.content[0].text` с `isError: true`), а не как JSON-RPC ошибка. Для неизвестного `action` текст перечисляет поддерживаемые действия:

```json
{"error": "Unknown action 'invalid_action_name_xyz' for crm_task. Available actions: list, get, get_by_key, create, ..."}
```

`-32603 Internal error` в ответе на `tools/call` означает внутренний сбой, а не способ сообщить о неверном действии.
...
| ``tropatt://server/toolsets`` | Профили toolset и счетчики инструментов | application/json | auth | 0.9 |
| ``tropatt://user/current`` | Текущий пользователь и права | application/json | auth | 0.9 |
| ``tropatt://server/api-map`` | Карта API-доменов | text/markdown | auth | 0.8 |
| ``tropatt://server/api-endpoints`` | Инвентарь REST endpoints ядра (из routes.php) | application/json | auth (settings.manage) | 0.75 |
