...
Права tool-ов никогда не шире соответствующего REST-маршрута. Инструменты idea-workflow и knowledge-AI вызываются напрямую на контроллере, поэтому каждый объявляет permission своего маршрута: AI debug требует `ai.admin`, чтение — соответствующий `*.view`, запись — соответствующий `*.manage`.
