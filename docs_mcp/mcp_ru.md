...
| MCP tools | ~617 в полном каталоге; `tools/list` по умолчанию возвращает core-профиль (~24 тула: 8 mega-tools + 16 regular) |
| MCP resources | 5 |
| MCP prompts | 0 |

---

## Mega-tools (консолидированные инструменты по намерению)

Core-профиль использует **mega-tools** — консолидированные инструменты, покрывающие целый домен через параметр `action`, заменяя десятки CRUD-тулов. Каждый mega-tool имеет подробное описание: когда использовать, что вернёт, как обращаться при ошибках.

| Mega-tool | Домен | Действия |
|-----------|-------|----------|
...
Полный каталог MCP велик (600+ инструментов). Чтобы не загружать его целиком в каждую сессию агента, `tools/list` по умолчанию возвращает курируемый профиль **`core`** (24 инструмента: профиль, поиск, дашборд, уведомления, активность и базовые чтения/создания по основным сущностям). Доступны профили по доменам:

| Профиль | Охват |
|---------|-------|
| `core` (по умолчанию) | Профиль, поиск, дашборд, уведомления, активность, базовые чтения задач/проектов/людей/БЗ/времени |
...
|---------|-------------:|------------:|
| `core` (по умолчанию) | 24 | ~2,5K токенов |
...
Полный каталог примерно в 10 раз дороже профиля `core` по умолчанию, поэтому для сессий, сфокусированных на одном домене, лучше узкий профиль.
