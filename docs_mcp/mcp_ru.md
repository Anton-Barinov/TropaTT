...
| MCP tools | ~607 в полном каталоге; `tools/list` по умолчанию возвращает core-профиль (~24 тула: 8 mega-tools + 16 regular) |
| MCP resources | 5 |
| MCP prompts | 0 |
...
Полный каталог MCP велик (600+ инструментов). Чтобы не загружать его целиком в каждую сессию агента, `tools/list` по умолчанию возвращает курируемый профиль **`core`** (~53 инструмента: профиль, поиск, дашборд, уведомления, активность и базовые чтения/создания по основным сущностям). Доступны профили по доменам:
