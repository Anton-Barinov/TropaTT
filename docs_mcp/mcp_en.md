...
Tools are never laxer than the equivalent REST route. The idea-workflow and knowledge-AI tools are dispatched directly to their controller, so each one declares the permission of its REST route: AI debug tooling requires `ai.admin`, read helpers require the matching `*.view`, and writes require the matching `*.manage`. Mega-tool *actions* are checked the same way: `crm_knowledge`, `crm_people`, `crm_crm`, `crm_ai` and `crm_admin` accept any of several permissions at tool level, so every action carries its own requirement (`crm_knowledge/reindex` → `settings.manage`, `crm_time/list_holidays` → `settings.manage`, `crm_task/list_approvals` → `approval.manage`, `crm_project/list_cabinet_projects` → `client.manage`). An action whose permission is not declared is rejected. Webhook subscriptions (`crm_admin` webhook actions) receive the core events (`task.created`, `project.updated`, `comment.added`, …) automatically, signed with HMAC-SHA256 and with secrets/financial fields stripped from the payload.
