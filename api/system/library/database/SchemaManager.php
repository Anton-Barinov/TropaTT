...
final class SchemaManager
{
    ...
    public function createSchema(PDO $pdo, string $driver): void
    {
        ...
        $tables = [
            ...
            "CREATE TABLE IF NOT EXISTS users (id {$id}, public_id VARCHAR(64) UNIQUE, login VARCHAR(120) UNIQUE, email VARCHAR(190), password_hash VARCHAR(255), auth_token_hash VARCHAR(255), full_name VARCHAR(255), locale VARCHAR(16), is_active {$bool} DEFAULT 1, is_root {$bool} DEFAULT 0, created_by_user_id INTEGER NULL, created_at {$dt}, updated_at {$dt}, deleted_at {$dt} NULL, cost_rate DECIMAL(12,2) DEFAULT NULL, bill_rate DECIMAL(12,2) DEFAULT NULL)",
            ...
            "CREATE TABLE IF NOT EXISTS tags (id {$id}, public_id VARCHAR(64) UNIQUE, code VARCHAR(64), title VARCHAR(255), color VARCHAR(32), description TEXT, created_at {$dt})",
            ...
            "CREATE TABLE IF NOT EXISTS recurring_rules (id {$id}, public_id VARCHAR(64) UNIQUE, title VARCHAR(255) NULL, entity_type VARCHAR(64), entity_public_id VARCHAR(64), rrule {$text}, is_active {$bool} DEFAULT 1, last_processed_at {$dt} NULL, created_at {$dt}, updated_at {$dt})",
