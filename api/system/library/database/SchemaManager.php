...
final class SchemaManager
{
    ...
    public function createSchema(PDO $pdo, string $driver): void
    {
        ...
        $tables = [
            ...
            "CREATE TABLE IF NOT EXISTS tasks (id {$id}, public_id VARCHAR(64) UNIQUE, project_id INTEGER NULL, title VARCHAR(255), description {$text}, status_code VARCHAR(64), priority_code VARCHAR(64), due_at {$dt} NULL, start_at {$dt} NULL, end_at {$dt} NULL, assignee_user_id INTEGER NULL, creator_user_id INTEGER, archived_at {$dt} NULL, deleted_at {$dt} NULL, created_at {$dt}, updated_at {$dt}, row_version INTEGER DEFAULT 1)",
            ...
            "CREATE TABLE IF NOT EXISTS recurring_rules (id {$id}, public_id VARCHAR(64) UNIQUE, entity_type VARCHAR(64), entity_public_id VARCHAR(64), rrule {$text}, is_active {$bool} DEFAULT 1, created_at {$dt}, updated_at {$dt})",
            "CREATE TABLE IF NOT EXISTS recurring_instances (id {$id}, public_id VARCHAR(64) UNIQUE, rule_id INTEGER, entity_public_id VARCHAR(64), generated_at {$dt}, created_at {$dt})",
