...
final class AiJobsRuntimeCompatibilityMigration implements MigrationInterface
{
    ...
    {
        return '20260426_000033_ai_jobs_runtime_compatibility';
    }

    public function description(): string
    {
        return 'Ensure ai_jobs compatibility columns for cron/runtime reads';
    }

    public function up(PDO $pdo, string $driver): void
    {
        if (!$this->tableExists($pdo, $driver, 'ai_jobs')) {
            return;
        }

        $this->ensureColumn($pdo, $driver, 'ai_jobs', 'scope_type', 'VARCHAR(64) NULL');
        $this->ensureColumn($pdo, $driver, 'ai_jobs', 'scope_public_id', 'VARCHAR(64) NULL');
        $this->ensureColumn($pdo, $driver, 'ai_jobs', 'idempotency_key_hash', 'VARCHAR(255) NULL');
        $this->ensureColumn($pdo, $driver, 'ai_jobs', 'started_at', 'DATETIME NULL');
        $this->ensureColumn($pdo, $driver, 'ai_jobs', 'finished_at', 'DATETIME NULL');
        $this->ensureColumn($pdo, $driver, 'ai_jobs', 'updated_at', 'DATETIME NULL');
    }
